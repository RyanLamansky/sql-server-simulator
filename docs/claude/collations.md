...
- `Equals` is `Compare == 0` and `GetHashCode` hashes the primary+secondary key, so DISTINCT / GROUP BY stay consistent with ORDER BY. Equality keeps every symbol significant (only trailing spaces fold, at the `SqlValue` layer), so `'co-op' = 'coop'` is false.
