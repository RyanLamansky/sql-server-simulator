...
- **Temp tables created inside dynamic SQL aren't scoped to the dynamic batch.** Real drops a `#temp` created inside an `sp_executesql` / `EXEC('…')` call when that dynamic batch returns (a following statement sees Msg 208); the simulator's temp tables are flat session state (`Connection.TempTables`), so a dynamic-SQL-created `#temp` persists for the session.
  Surfaced by tedious, whose `execSql` runs every statement through `sp_executesql`, so `execSql("create table #x…")` then `execSql("select … from #x")` succeeds on the sim but fails on real; a true SQLBatch (tedious `execSqlBatch`) persists on both.
  The divergence is in the lenient direction (real-valid code still runs on the sim) and modeling it means tracking each temp table's creating batch and dropping it at dynamic-batch exit — a standalone engine change, not endpoint-local; deferred (see [`docs/claude/backlog.md`](backlog.md), [`temp-tables.md`](temp-tables.md)).
