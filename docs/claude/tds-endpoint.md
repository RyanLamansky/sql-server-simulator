...
Both listen methods also take a **`SimulatedNetworkListenerOptions`** overload (`Port`, default 1433 matching the int overloads; `BindAddress`; `ServerCertificate`, default null = the process-wide default certificate): a supplied certificate must carry a private key (`ArgumentException` otherwise) and **stays owned by the caller — the listener never disposes it**.
...
Connection-string requirements: `TrustServerCertificate=true` (the endpoint presents an in-memory self-signed cert, unless the options overload supplied one — a CA-trusted supplied certificate makes the flag unnecessary) and credentials per the enforcement rule below (any credentials when no logins exist).

**The default certificate is created once per process and shared by every listener that does not supply one** (`TdsServerCertificate.Shared`, a lazily-materialized RSA-2048 self-signed cert).
Generating it costs far more than standing up the listener around it, so a host creating many listeners — a test suite with one per case — would otherwise pay for a key pair per listener; sharing also matches a real server, which presents one identity rather than a fresh one per endpoint.
It is never disposed (a listener disposing it would break every other listener), which is why `SimulatedNetworkListener` tracks no certificate ownership: the private-key certificate it holds is always either the caller's or the process-wide default, and only the per-listener public-part copy exposed as `ServerCertificate` is disposed with the listener.
...
Because the default certificate is process-wide, one pin file serves every listener (the shape `StrictEncryptionTests` uses: the shared certificate's public part written once to a fixed-name file in the OS temp directory).
...
- **SQLBatch** (type 1): ALL_HEADERS skipped via its leading length DWORD; UCS-2 text executed on the session connection.
  ...
  A DONE's **`CurCmd`** names the kind of statement that produced it, which is what decides whether a client folds the count into `RecordsAffected` — SqlClient sums every DONE's count *except* a SELECT's, since a SELECT reports rows returned rather than rows affected.
  Real tags a plain SELECT and a cursor `FETCH` `0x00C1`, and `SELECT INTO` / `INSERT` / `DELETE` / `UPDATE` / `MERGE` their own kinds (`0x00C2` / `0x00C3` / `0x00C4` / `0x00C5` / `0x0117`); the simulator writes `0x00C1` for a result set and leaves every other kind `0` (`Tds.CmdSelect`, off a cleartext wire capture, 2026-08-03).
  A DML statement whose `OUTPUT` clause returns rows to the client is the one result set that goes out unclassified, because its count *is* a rows-affected count.
  **`SET NOCOUNT ON`** suppresses `DONE_COUNT` on every statement's DONE, result sets included, while the row count itself stays in the token (real's shape) — so an ODBC/pyodbc driver skips the DML's rowcount result and advances to a trailing `SELECT SCOPE_IDENTITY()` (the mssql-django / Django identity pattern; without it the driver stalls on the INSERT's rowcount and never reaches the SELECT), and SqlClient's `ExecuteNonQuery` returns -1.
  The suppression is recorded on the outcome when the statement runs rather than read off the session flag when the DONE is written: the next statement has already executed by then (the outcome enumerator is pulled one ahead), so its own `SET NOCOUNT` would otherwise decide this statement's DONE — and a procedure body's `SET NOCOUNT` reverts before its outcomes reach the writer at all.
...
- **PRINT / low-severity RAISERROR and every other informational message**: each arrives as an info outcome in the engine's stream and goes out as one INFO token, ahead of the next outcome's tokens — except Msg 8153, which closes the statement it follows and so is written before that statement's DONE, where real sends it.
...
A **NOT NULL** fixed-width bulk destination arrives as the FIXEDLENTYPE token with an unprefixed value, the read mirror of the result-column rule, so the legacy-scaled families (`money`, `datetime`, `smalldatetime`) each decode through their own fixed-width arm — `BulkCopyTests.NotNullFixedWidthColumns_DecodeFromTheFixedLenForm` drives them, the nullable sibling test drives the N-variants.
An `xml` **TVP** column is the only route to the decoder's XML arm: a bulk-copy `xml` destination travels as the MAX-string form instead.
...
- **`sql_variant` (`0x62`)** — TYPE_INFO is a 4-byte max length (ignored); the value is a 4-byte total length (**0 = NULL**) then the MS-TDS §2.2.5.5.3 body (base-type token + property-byte count + property bytes + inner value), the read mirror of `TdsTypeCodec.BuildVariantBody`, decoded into the inner `SqlValue` and wrapped by `SqlValue.FromVariant` so `SQL_VARIANT_PROPERTY(@p,'BaseType')` reports the sent type.
  ...
  `DateTimeOffset` → `datetimeoffset`, which is the one base type SqlClient sends that its CLR-type table doesn't reach through some other value.
  The **body reader and writer are pinned against each other over every base type** by `SqlVariantBodyRoundTripTests` (Tests.Internal), which is what covers the arms SqlClient never writes — `smalldatetime`, `datetime2` and `varchar` all have legal wire forms another driver can send while SqlClient picks a different base type for the same CLR value.
  The blank-padded types are **their own base types both ways**: `char` (`0xAF`) / `nchar` (`0xEF`) / `binary` (`0xAD`) are written and read as declared rather than collapsed into the varying spelling, matching what real reports back from `SQL_VARIANT_PROPERTY(…, 'BaseType')`.
...
- **Nullability (fNullable)**: `SimulatedQueryResult.ColumnNullability` drives the per-column flag (first flags byte 0x09 nullable / 0x08 not).
  It's populated by the SELECT projection — the FROM-less shape (`BuildSynthesizedSqlRow`) and every FROM shape however many sources it joins (`BuildSqlProjection`) — via the same `Expression.ResultIsNullable` inference SELECT INTO uses, and a set operation combines its branches' answers (`CombinedNullability`: UNION nullable when either branch is, INTERSECT when both are, EXCEPT as the left branch is — probed 2026-09-24); every other producer leaves it null = claim all-nullable.
  A join reads each source against a **per-source NULL-fill map** (`Selection.Execution.cs`'s `NullFilledSources`, mirroring the fold the executor runs): LEFT and OUTER APPLY NULL-fill the right operand, RIGHT the left spine, FULL both, and a parenthesized join group recurses.
  So an INNER join preserves both operands' base nullability while an outer join's NULL-filled side reads nullable however its base column is declared — probe-confirmed against SQL Server 2025 through `sys.dm_exec_describe_first_result_set`, and the same map is what keeps a `SELECT … INTO` from declaring a NULL-extended destination column NOT NULL.
  A GROUP BY that spells any grouping-set form (`ROLLUP` / `CUBE` / `GROUPING SETS` / `WITH ROLLUP` / `WITH CUBE`) makes **every** column reference nullable, even one present in every set — real's rule, probed 2026-09-23 — since a rolled-up row carries the grouped-away column NULL (`FromClause.GroupingSetsWritten`).
  A view's and an inline TVF's `sys.columns` rows read the same inference, so their metadata and a direct read of their body agree.
  **The inference's rule set is documented on `Expression.ResultIsNullable` itself**, and pinned by `ResultNullabilityTests` (the exhaustive matrix, read off the SELECT INTO surface) plus `ColumnNullabilityWireTests` (the wire flag, one pin per cluster).
  Structurally, a direct ref preserves the base column's declaration, a non-NULL literal is NOT NULL, and a parenthesization / `COLLATE` / `AS alias` passes its operand's answer through.
  Per built-in it is an idiosyncratic probed table with no rule behind it: `CEILING` / `FLOOR` / `ROUND` / `SIGN` / `RADIANS` / `GREATEST` / `LEAST` / the six `…FROMPARTS` propagate their arguments' nullability; `PI` / the `GETDATE` family / `CONCAT` / `ROWCOUNT_BIG` / `MIN_ACTIVE_ROWVERSION` / `CURRENT_REQUEST_ID` / `APPLOCK_TEST` and every `@@`-constant but `@@IDENTITY` are unconditionally NOT NULL; everything else claims nullable, including the near neighbours (`ABS`, `POWER`, `RAND`, `NEWID`, every date and string scalar, `CAST` / `CONVERT`, every aggregate).
  Arithmetic `+ - * / %` claims nullable even over two NOT NULL operands, while the bitwise operators and both concatenation spellings (`+` over strings / binaries, `||`) propagate.
  The CASE family (`CASE` / `IIF` / `COALESCE` / `NULLIF`) is the OR over its arms *after* real's constant fold, which drops an arm whose condition folds FALSE and truncates at one that folds TRUE.
  A surviving arm — and every `GREATEST` / `LEAST` argument — then answers for the conversion the arm unification put on it, reading nullable whenever that conversion could alter the value (`SqlType.ConversionPreservesEveryValue`): `COALESCE(<decimal(9, 2) col>, 0)` is nullable because the unification narrows to the column's own width (the literal's *value* is one digit wide) and then converts the literal from its declared `int`, whose ten integral digits don't fit the column's seven, while `COALESCE(<decimal(9, 2) col>, 0.0)` and `COALESCE(<decimal(9, 2) col>, <int col>)` — the latter widening the result to `decimal(12, 2)` — are NOT NULL.
  Loss, not failure, is what the test asks: `decimal` → `float` and `datetime` → `datetime2` can't raise and both read nullable, because a scaled decimal has no exact binary form and `datetime`'s 1/300-second grid lands on no tick grid, where `decimal(9, 0)` → `float` (nine integral digits inside float's 2^53) and `smalldatetime` → `datetime` (a whole minute is a multiple of 1/300 s) are NOT NULL.
  `date` → `datetime` / `smalldatetime` is nullable on range alone (they start in 1753 / 1900).
  Set operators and a VALUES constructor unify the same way and do *not* carry the rule, and `ISNULL` takes argument one's type outright so nothing converts there — both probe-confirmed.
  One shipped divergence: `CURSOR_STATUS` propagates its arguments here where real is unconditionally NOT NULL, because the simulator's `CURSOR_STATUS(scope, NULL)` returns NULL and a NOT NULL fixed-width column has no NULL wire form — the general rule for this flag being that a NOT NULL claim must be one the value can't contradict, since an over-claim corrupts the stream rather than merely mis-describing it.
  ...
  The rest of that flags byte follows the column's character as real sets it (`SimulatedQueryResult.ColumnWireFlags`, computed once per `Selection`): a column read keeps its table column's — `fIdentity` and read-only for an identity column, `fComputed` and read-only for a computed one, read-only for a rowversion, updatability-unknown (`0x08`) otherwise — traced through views, joins, derived tables and CTEs; a scalar expression is `fComputed` and read-only; an aggregate, a window function and a set operation's column are read-only with neither flag (captured 2026-09-26).
  SqlClient turns those bits into `GetSchemaTable`'s `IsIdentity` / `IsAutoIncrement` / `IsReadOnly`, which `DataTable.Load` copies into its columns.
  A derived table's or CTE's columns carry the inner projection's inferred nullability, so a read through one keeps it — real's `SELECT … INTO` does too (probed 2026-09-26).
...
- **Strings**: `varchar/char` value bytes use the code page implied by the advertised collation (`TdsCollationCodec.WireEncoding`) so the client's decode round-trips; `nvarchar/nchar/sysname` are UCS-2.
  ...
  **A bounded string column's projected width is a wire contract, not just metadata**: COLMETADATA advertises the declared maximum and the ROW value carries its own `USHORT` byte length, so a projection narrower than the value it computes emits a length prefix past the advertised maximum and the stream desyncs — the native ODBC driver reports "Protocol error in TDS stream" and the connection is unusable afterwards.
  A width-deriving expression that can't bound its result therefore falls back to the family container (which advertises 8000 / 4000) rather than to a computed width, and the length-0 container form is what `NationalMaxLength` / `VariableMaxLength` render as that advertised maximum.
  `StringLiteralWidthWireTests` pins the widths the endpoint advertises for the whole width algebra, including the container cases.
