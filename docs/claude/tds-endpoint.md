...
- **Nullability (fNullable)**: `SimulatedQueryResult.ColumnNullability` drives the per-column flag (first flags byte 0x09 nullable / 0x08 not).
  It's populated by the no-join SELECT projection with at most one source — the single-source shape (`BuildSqlProjection`) and the FROM-less shape (`BuildSynthesizedSqlRow`) — via the same `Expression.ResultIsNullable` inference SELECT INTO uses; every other producer leaves it null = claim all-nullable, including joined shapes (outer joins NULL-fill a side, so base-column nullability would over-claim NOT NULL — a real server marks inner-join base columns NOT NULL, a benign divergence).
