...
- **Nullability (fNullable)**: `SimulatedQueryResult.ColumnNullability` drives the per-column flag (first flags byte 0x09 nullable / 0x08 not).
  It's populated by the SELECT projection — the FROM-less shape (`BuildSynthesizedSqlRow`) and every FROM shape however many sources it joins (`BuildSqlProjection`) — via the same `Expression.ResultIsNullable` inference SELECT INTO uses; every other producer leaves it null = claim all-nullable.
