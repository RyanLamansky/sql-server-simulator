...
**Column sources remember the name** (`HeapColumn.SpelledNumeric`, `VariableSlot.SpelledNumeric`): a column declared `numeric` (CREATE TABLE, ALTER TABLE ADD / ALTER COLUMN), one a `SELECT … INTO` or a view's projection derived as numeric, a derived table's, CTE's or `VALUES` constructor's numeric column, and a variable declared `numeric`.
...
A set operation's column and a `VALUES` constructor's column take the name of their **first decimal-family** branch or row — `SELECT 2.0 UNION ALL SELECT CAST(1 AS decimal(5, 1))` is numeric and the reverse decimal, while an `int` first branch leaves the choice to the next (probed 2026-09-24) — and an aggregate over a numeric-spelled column keeps the name.
**Not modeled yet**: a reference *inside* a scalar expression (`n + d`, `n * 2`, and so a computed column over `n`), a procedure's or function's numeric parameter and return type, and an alias type over numeric still read `decimal`; so does the operand name in a type-pair message.
