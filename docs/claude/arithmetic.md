...
**Column sources remember the name** (`HeapColumn.SpelledNumeric`, `VariableSlot.SpelledNumeric`): a column declared `numeric` (CREATE TABLE, ALTER TABLE ADD / ALTER COLUMN), one a `SELECT … INTO` or a view's projection derived as numeric, a derived table's or CTE's numeric column, and a variable declared `numeric`.
...
**Not modeled yet**: a reference *inside* an expression (`n + d`, `n * 2`, and so a computed column over `n`), a `VALUES` constructor's column, a procedure's or function's numeric parameter and return type, and an alias type over numeric still read `decimal`; so does the operand name in a type-pair message.
