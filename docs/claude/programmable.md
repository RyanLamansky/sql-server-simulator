...
- **A declaration may carry a constant default**, ahead of `OUTPUT` as a procedure parameter's does (`@p int = 5 OUTPUT`): a literal, a signed number, `NULL` or `DEFAULT` (read as NULL); anything else is a syntax error at it, and a variable Msg 137 (probed 2026-09-25).
  The declarations parse the way real reads them, as the parenthesized list Msg 8178 prints, so a list that ends early is Msg 102 near its closing `)` and text after it Msg 4124.
- **A bound value is assigned to its parameter as `SET` assigns a variable**, so one past the declared width is cut to it — `@a nvarchar(2)` bound `N'abcd'` reads `ab` — for an `sp_executesql` declaration as for a procedure parameter (probed 2026-09-25).
- **Every declared parameter without a default must be supplied** — **Msg 8178**, quoting the two argument strings verbatim as `'(<declarations>)<statement>'`, spacing and all.
