...
7. **WWI-discovered gap inventory** (candidates for follow-up bundles, ranked by Skipped count):
   - `SqlExtendedProperty` (83): extended properties on the 2 still-deferred computed columns + on table-type columns + on filegroup. (Was 89 before computed columns landed — 6 host columns unblocked their attached extended properties.)
   - `SqlComputedColumn` (2 remaining): both reference `json_query` which the simulator doesn't yet model (only `JSON_VALUE` / `JSON_MODIFY` / `OPENJSON` ship — see `docs/claude/json.md`). Adding `JSON_QUERY` to the scalar-function dispatcher would unblock both.
   - `SqlProcedure` (3 in WWI): all three reference the undefined system type `sysname` (e.g. `EXEC sp_addrolemember @rolename sysname, @membername sysname`). `sysname` is real SQL Server's alias for `nvarchar(128) NOT NULL` — adding it to the simulator's built-in type registry would unblock these.
   - `SqlIndex` (3): all three are filtered indexes on computed columns (`IX_Purchasing_SupplierTransactions_IsFinalized`, `IX_Sales_CustomerTransactions_IsFinalized`, `IX_Sales_Invoices_ConfirmedDeliveryTime`). Computed columns land in phase 8, indexes in phase 5 — moving indexes later tripped an unrelated UDF-resolution gap in ALTER TABLE ADD AS for AW. Resolving the UDF gap (or splitting indexes into two passes — pre-computed and post-computed) would unblock these.
   - `SqlCheckConstraint` (2): both are JSON `CHECK ISJSON(…)=1` patterns.
   - `SqlPermissionStatement` (2): same deferral as AW — GRANT/REVOKE wire-up.
   - `SqlView` (1): `[Website].[VehicleTemperatures]` uses `DECOMPRESS` (Microsoft's COMPRESS/DECOMPRESS gzip pair); function not modeled.
   - `SqlScalarFunction` (1): `[Website].[CalculateCustomerPrice]` uses `SCHEMABINDING` / `WITH EXECUTE AS` beyond `RETURNS NULL ON NULL INPUT`.
   - `SqlFilegroup` (1): trivially skippable.
...
8. **Real XPath/XQuery + OGC method evaluation + full-text search** as separate post-baseline initiatives, each promoted from "parses cleanly, throws at execute" to first-class as bundles complete.
...
WideWorldImporters-Standard (Microsoft's modern sample DB) also loads end-to-end after the 2026-05-15 bundles: 48/48 tables, 26/26 sequences, 9/9 roles, 4/4 table types, 6/8 computed columns (2 deferred for unmodeled `json_query`), 4.7M rows, zero per-element load failures (the `"Deferred:"`-prefixed Skipped entries are expected). Remaining gaps cataloged in step 7 of the order-of-operations checklist above.
