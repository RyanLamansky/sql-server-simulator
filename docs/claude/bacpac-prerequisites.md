...
8. **WWI-discovered gap inventory** (candidates for follow-up bundles, ranked by Skipped count):
   ...
   - ~~`SqlProcedure` (was 3)~~: shipped in the sysname bundle (step 5 above) — all 3 now load.
...
9. **Real XPath/XQuery + OGC method evaluation + full-text search** as separate post-baseline initiatives, each promoted from "parses cleanly, throws at execute" to first-class as bundles complete.
...
WideWorldImporters-Standard (Microsoft's modern sample DB) also loads end-to-end after the 2026-05-15 bundles: 48/48 tables, 26/26 sequences, 9/9 roles, 4/4 table types, 6/8 computed columns (2 deferred for unmodeled `json_query`), 42/42 procedures, 4.7M rows, zero per-element load failures (the `"Deferred:"`-prefixed Skipped entries are expected). Remaining gaps cataloged in step 8 of the order-of-operations checklist above.
