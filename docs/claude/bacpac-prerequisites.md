...
9. **WWI-discovered gap inventory** (candidates for follow-up bundles, ranked by Skipped count):
   - `SqlExtendedProperty` (81): extended properties on table-type columns + on filegroup + on a handful of computed-col-adjacent columns the loader doesn't yet host-route. (Was 89 before computed columns landed → 83 after JSON_QUERY → 81 after the JSON_QUERY bundle.)
   - ~~`SqlComputedColumn`~~: shipped in step 6 (JSON_QUERY).
...
10. **Real XPath/XQuery + OGC method evaluation + full-text search** as separate post-baseline initiatives, each promoted from "parses cleanly, throws at execute" to first-class as bundles complete.
...
WideWorldImporters-Standard (Microsoft's modern sample DB) also loads end-to-end after the 2026-05-15 bundles: 48/48 tables, 26/26 sequences, 9/9 roles, 4/4 table types, 8/8 computed columns, 42/42 procedures, 4.7M rows, zero per-element load failures. Remaining gaps cataloged in step 9 of the order-of-operations checklist above.
