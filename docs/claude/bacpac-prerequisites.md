...
11. **Real XPath/XQuery + OGC method evaluation + full-text search** as separate post-baseline initiatives, each promoted from "parses cleanly, throws at execute" to first-class as bundles complete.
...
WideWorldImporters-Standard (Microsoft's modern sample DB) also loads end-to-end after the 2026-05-15 bundles: 48/48 tables, 26/26 sequences, 9/9 roles, 4/4 table types, 8/8 computed columns, 42/42 procedures, 94/94 indexes, 3/3 views, 1/1 scalar function, 4.7M rows. Remaining Skipped surface is narrow: 81 SqlExtendedProperty entries (cascade — hosts not host-routed yet), 2 SqlPermissionStatement (encryption-key GRANTs), 1 SqlCheckConstraint (boolean-parser gap on paren-wrapped value subexpressions), 1 SqlDatabaseOptions (non-default collation). Detailed catalog in step 10 of the order-of-operations checklist above.
