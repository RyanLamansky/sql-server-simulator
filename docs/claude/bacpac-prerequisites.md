...
6. **WWI-discovered gap inventory** (candidates for follow-up bundles, ranked by Skipped count):
   - `SqlExtendedProperty` (89): extended properties on computed columns — these need the corresponding host elements to land first.
   - `SqlComputedColumn` (8): same deferral as AW — needs the post-functions ALTER TABLE pass.
   - `SqlProcedure` (3 in WWI): the first one (`[Application].[AddRoleMemberIfNonexistent]`) likely needs `EXECUTE sp_addrolemember` system-procedure modeling. (Was 6 before the SqlTableType dispatch landed — 3 of the previous failures were procs with TVP parameters whose underlying table types weren't declared.)
...
WideWorldImporters-Standard (Microsoft's modern sample DB) also loads end-to-end after the 2026-05-15 resilience + sequences + roles + table types bundle: 48/48 tables, 26/26 sequences, 9/9 roles, 4/4 table types, 4.7M rows, zero per-element load failures. Remaining gaps cataloged in step 6 of the order-of-operations checklist above.
