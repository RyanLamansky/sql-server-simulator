...
13. **WWI-discovered gap inventory** (remaining categories, ranked by Skipped count):
   ...
   - `SqlDatabaseOptions` (1): WWI declares Collation `Latin1_General_100_CI_AS`; the simulator only models the default `SQL_Latin1_General_CP1_CI_AS`. The skip-with-diagnostic stays on Skipped; modeling non-default collations is its own multi-area project (sort orders, comparison rules, LIKE behavior, sys.fn_helpcollations rows, COLLATE expression dispatch).
...
14. **Real XPath/XQuery + OGC method evaluation + full-text search** as separate post-baseline initiatives, each promoted from "parses cleanly, throws at execute" to first-class as bundles complete.
...
WideWorldImporters-Standard (Microsoft's modern sample DB) also loads end-to-end after the 2026-05-15+16 bundles: 48/48 tables, 26/26 sequences, 9/9 roles, 4/4 table types, 8/8 computed columns, 7/7 CHECK constraints (including the paren-wrapped `((value_expr) = (1))` shape that the 2026-05-16 boolean-parser fix unlocked), 42/42 procedures, 94/94 indexes, 3/3 views, 1/1 scalar function, 2/2 encryption-key GRANTs (the 2026-05-16 SqlPermissionStatement dispatcher), **414/414 extended properties** (the 2026-05-16 SqlIndexBase + SqlConstraint host-routing bundle cleared the last 81-element category), 4.7M rows. Only Skipped surface remaining: 1 SqlDatabaseOptions (non-default collation — a substantive feature requiring multi-collation support, deferred). Detailed catalog in step 10 of the order-of-operations checklist above.
