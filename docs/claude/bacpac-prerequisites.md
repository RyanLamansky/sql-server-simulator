...
12. ~~**WWI-Full Phase 1 (partitioning + columnstore + Latin1_General_CI_AS whitelist)**~~ — shipped 2026-05-16. WideWorldImporters-Full is the larger sibling sample to WWI-Standard; same 48 tables + 4.7M rows but with three new structural element kinds (`SqlPartitionFunction`, `SqlPartitionScheme`, `SqlColumnStoreIndex`) and 2 columns declaring `Latin1_General_CI_AS`. (i) Partitioning + columnstore land as silent skips in phase 1, matching `SqlFilegroup`'s pattern — all four are storage-organization decorations with no semantic effect on row-store query results, so surfacing them on `Skipped` would create category noise. (ii) `Latin1_General_CI_AS` (the older 80-collation matching `Latin1_General_100_CI_AS`'s family) joins `Collation.Recognized`; the 2 `Warehouse.VehicleTemperatures` columns that declared it now round-trip through `sys.columns.collation_name`. Loader survives WWI-Full with **1 remaining Skipped category** (`SqlProcedure` × 1 — `Website.RecordColdRoomTemperatures` natively-compiled procedure body, see step 13 below). Permanent regression tests: 3 new in `BacpacLoaderTests` (`Load_WWIFull_Loads_All_Tables`, `Load_WWIFull_Known_Gaps_Recorded_In_Skipped`, `Load_WWIFull_Latin1_CI_AS_Columns_Warning_Free`) + 1 new public-API test (`CollationMetadataTests.Column_Collate_Latin1_General_CI_AS_RoundTrips`).
13. ~~**Natively-compiled procedure body**~~ — shipped 2026-05-16. `ParseProcedureWithOptions` extended to accept `SCHEMABINDING` and `NATIVE_COMPILATION` (both parse-and-discard); `ContextualKeyword.Native_Compilation` added. The `BEGIN ATOMIC` dispatch arm in `Simulation.cs` flipped from `throw NotSupportedException` to a new `ParseBeginAtomicBlock` in `Simulation.IfBlock.cs` that consumes `BEGIN ATOMIC [WITH (...)]`, balanced-paren-skips the options block (TRANSACTION ISOLATION LEVEL / LANGUAGE / DATEFORMAT / DATEFIRST / DELAYED_DURABILITY — no semantic effect in the simulator), then dispatches statements like `ParseBeginBlock` until `END`. WWI-Full's `Website.RecordColdRoomTemperatures` (the sole natively-compiled SP in either sample) now loads. **WWI-Full reaches zero `Skipped` categories.** Permanent regression tests: 5 new in `StoredProcedureTests` (`Create_With_NativeCompilation_Schemabinding_Parses`, `BeginAtomic_Body_Runs`, `BeginAtomic_With_TryCatch_Body`, `BeginAtomic_Without_With_Options_Block_Parses`, `BeginAtomic_Empty_Body_RaisesSyntax_On_Exec`) + existing `BeginAtomic_NotSupported` flipped to `BeginAtomic_AtBatchTopLevel_DispatchesBody` + `Load_WWIFull_Known_Gaps_Recorded_In_Skipped` updated to assert `Skipped` is empty.
14. ~~**Geography WKB → WKT decoder**~~ — shipped 2026-05-16. New `SpatialWkbDecoder.TryDecodeSimplePoint` in `Storage/Bacpac/` covers Microsoft's spatial UDT simple-point wire shape (4-byte SRID + 1-byte version + 1-byte properties bitfield + 16-byte coordinates = 22 bytes). Geography axis inversion (binary stores lat/long, WKT prints long/lat per OGC) honored. Anything outside the simple-point shape — LineString / Polygon / MultiPolygon / FullGlobe / Z or M coordinates / unknown version — returns null and the row loader falls back to `SqlValue.Null` (resilient: complex shapes don't fail the whole BCP file). AW's 19,614 `Person.Address.SpatialLocation` rows are all simple points and decode end-to-end. Permanent regression tests: 7 in `SpatialWkbDecoderTests` (axis inversion, round-trip precision, truncated payload, non-simple-point properties, Z/M bits, unknown version) + updated `Load_AW_Geography_Column_Decodes_To_Point_Wkt` asserting 0 NULL rows and the `POINT (` WKT prefix.
15. **Hierarchyid byte-identical CAST encoding** — the BCP wire decoder ships (covers AW's [0..79] positive-ordinal envelope; throws `NotSupportedException` on negative ordinals + ordinals ≥ 80 + dotted sub-ordinals so a follow-up bundle can extend cleanly), but the simulator's own `HierarchyIdSqlType.Encode` / `Decode` still uses its segment-array-LE internal form. Replacing both with the documented OrdPath encoding makes cross-engine CAST byte equality hold round-trip; the wire decoder gives the read half for free, the symmetric encoder is the remaining work.
16. **WWI-discovered gap inventory** (remaining categories, ranked by Skipped count):
   ...
   - ~~`SqlColumnStoreIndex` / `SqlPartitionFunction` / `SqlPartitionScheme` (3 + 2 + 2 in WWI-Full)~~: shipped 2026-05-16 — silent skip in phase 1 (step 12 above).
   - ~~`SqlProcedure` (1 in WWI-Full)~~: shipped 2026-05-16 — natively-compiled procedure body (step 13 above).
   - ~~**Temporal-table wiring**~~: shipped 2026-05-16. WWI's 17 system-versioned base tables (every `*_Archive` is a history sibling via the `TemporalSystemVersioningHistoryTable` relationship) now load with the link intact. Three loader changes: `TranslateSimpleColumn` emits `GENERATED ALWAYS AS ROW START / END` when `GeneratedAlwaysType` = 1/2; `EmitTable` appends `PERIOD FOR SYSTEM_TIME (startCol, endCol)` to the column list when both period columns are present; new phase 5 (`EmitDeferredSystemVersioning`) walks each SqlTable a second time and emits `ALTER TABLE base SET (SYSTEM_VERSIONING = ON (HISTORY_TABLE = name))` for every element carrying the temporal relationship. The simulator-side ALTER ... ON shape was added to `TryParseAlterTableSetSystemVersioning` in `Simulation.Alter.cs` (inverse of the existing OFF direction; accepts optional `, DATA_CONSISTENCY_CHECK = ON|OFF` parse-and-discard). `sys.tables` gained `temporal_type` (tinyint: 0=non, 1=history, 2=system-versioned), `temporal_type_desc` (nvarchar), and `history_table_id` (int) so the wire-up is inspectable from SQL. WWI verification: 17 base tables with `temporal_type=2`, 17 history tables with `temporal_type=1`, `FOR SYSTEM_TIME ALL` query against `Application.Cities` returns current + archive rows.
17. **Real XPath/XQuery + OGC method evaluation + full-text search** as separate post-baseline initiatives, each promoted from "parses cleanly, throws at execute" to first-class as bundles complete.
...
Loader baseline shipped (2026-05-15). `Simulation.FromBacpac` loads AdventureWorks2025 end-to-end with **100% row coverage** (760,167 / 760,167 rows, zero BCP-file failures). All four DDL/data type families ship: schema + constraints + indexes + programmable objects + extended properties + every BCP wire-format type (fixed-raw, 1-byte-prefix, 2-byte-prefix, 8-byte-prefix). Geography simple-point rows decode to `POINT (long lat)` WKT via `SpatialWkbDecoder.TryDecodeSimplePoint`; complex shapes (none in AW) fall back to NULL. Hierarchyid rows decode to canonical-string-equivalent segment arrays via `HierarchyIdWireDecoder`.
...
**WideWorldImporters-Full** (2026-05-16 bundles) is the larger sibling sample — same row volume + schema as Standard but adds partitioning, columnstore indexes, and one natively-compiled procedure. The loader survives with three new silent-skip categories (`SqlPartitionFunction`, `SqlPartitionScheme`, `SqlColumnStoreIndex`) joining `SqlFilegroup`'s pattern — all four are storage-organization decorations with no semantic effect on row-store query results, so surfacing them on `Skipped` would create noise. `Latin1_General_CI_AS` (the older 80-collation that WWI-Full's `Warehouse.VehicleTemperatures` declares on two columns) joins the `Collation.Recognized` whitelist. The natively-compiled `Website.RecordColdRoomTemperatures` SP also loads after the NATIVE_COMPILATION + BEGIN ATOMIC parser bundle (step 13). **WWI-Full reaches zero `Skipped` categories.** Regression tests landed in `BacpacLoaderTests.Load_WWIFull_*` (3 tests: full-load smoke + Skipped-census + Latin1_General_CI_AS warning-free round-trip).
