...
13. **Natively-compiled procedure body** — WWI-Full's `Website.RecordColdRoomTemperatures` is the only natively-compiled procedure in either sample; the loader-emitted `CREATE PROCEDURE` fails with `Incorrect syntax near 'NATIVE_COMPILATION'`. The body decoration shape is `WITH NATIVE_COMPILATION = ON, SCHEMABINDING, EXECUTE AS OWNER AS BEGIN ATOMIC WITH (TRANSACTION ISOLATION LEVEL = SNAPSHOT, LANGUAGE = N'us_english') <body> END`. Two parts need work: (i) the `WITH NATIVE_COMPILATION = ON` clause in the procedure-header parser (parse-and-discard since the simulator doesn't natively-compile anything); (ii) the `BEGIN ATOMIC WITH (...)` block (currently `NotSupportedException` at dispatch). `BEGIN ATOMIC` semantics are real transactions with statement-atomic rollback — most of the surface already ships through the implicit per-statement undo log; the work is parsing the `WITH (...)` options block + tying the explicit atomic frame to the existing log lifecycle.
14. **Geography WKB → WKT decoder** — Person.Address rows load with `SpatialLocation = NULL` (the 8-byte-prefix bytes are drained but not decoded). A future bundle could implement the simple-point case (4-byte SRID + 1-byte version + 1-byte properties + 16-byte X/Y doubles) which is all AW uses; full WKB shape support (LineString / Polygon / MultiPolygon / etc.) is a larger project.
...
16. **WWI-discovered gap inventory** (remaining categories, ranked by Skipped count):
   ...
   - `SqlProcedure` (1 in WWI-Full): natively-compiled procedure body (step 13 above).
...
**WideWorldImporters-Full** (2026-05-16 probe + bundle) is the larger sibling sample — same row volume + schema as Standard but adds partitioning, columnstore indexes, and one natively-compiled procedure. The loader survives with three new silent-skip categories (`SqlPartitionFunction`, `SqlPartitionScheme`, `SqlColumnStoreIndex`) joining `SqlFilegroup`'s pattern — all four are storage-organization decorations with no semantic effect on row-store query results, so surfacing them on `Skipped` would create noise. `Latin1_General_CI_AS` (the older 80-collation that WWI-Full's `Warehouse.VehicleTemperatures` declares on two columns) joins the `Collation.Recognized` whitelist. The only remaining `Skipped` category is **1 procedure** (`Website.RecordColdRoomTemperatures`) whose `WITH NATIVE_COMPILATION = ON, SCHEMABINDING, EXECUTE AS OWNER + BEGIN ATOMIC WITH (...)` body decoration isn't parsed — natively-compiled procedures need a body-decoration parser bundle covering both the `WITH NATIVE_COMPILATION` clause and the `BEGIN ATOMIC` block. Regression tests landed in `BacpacLoaderTests.Load_WWIFull_*` (3 tests: full-load smoke + Skipped-census + Latin1_General_CI_AS warning-free round-trip).
