...
The representative fallback fires only for non-empty grouping sets, preserving the ROLLUP/CUBE grand-total NULL; an *ungrouped* column reference still resolves to the representative (returning a row value rather than SQL Server's Msg 8120 compile error — a minor fidelity gap for illegal queries).
