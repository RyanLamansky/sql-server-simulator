...
Not modeled **in-process**: `IEnumerable<SqlDataRecord>` (the third documented TVP value type). `SqlDataRecord` lives in `Microsoft.Data.SqlClient.Server`; adding it in-process would require either a SqlClient dependency (load-bearing-no) or a reflection-based duck-typed path. **Over the TDS wire it works**: SqlClient serializes an `IEnumerable<SqlDataRecord>` (and a `DbDataReader`) to the same TVP wire form a `DataTable` produces, so all three documented source shapes bind through the listener — see the wire path below.

## TDS wire path

The TDS network endpoint decodes a `SqlDbType.Structured` parameter (RPC TYPE_INFO `0xF3`) and **feeds the same structured-parameter binding described above**: the decoded rows become a `TableValuedParameterData` carrier (a third source shape alongside `DataTable` / `IDataReader` in `IsTableValuedParameterValue`), the parameter's `TypeName` comes from the wire TVP_TYPENAME, and `MaterializeTvpRows` resolves + clones + inserts positionally exactly as the in-process path does. Wire form, sources, and error parity live in [`tds-endpoint.md`](tds-endpoint.md#tvp-parameters-sqldbtypestructured).

Constraint enforcement is shared by both entry points via `Simulation.InsertTableValuedParameterRow`: each row evaluates computed columns then enforces NOT NULL (Msg 515), CHECK (Msg 547), PRIMARY KEY / UNIQUE (Msg 2627), and UNIQUE indexes (Msg 2601) against the rows inserted so far before the heap write. This closed a pre-existing gap — the structured path previously did a direct `Heap.Insert` that skipped every constraint (probe-confirmed real SQL Server raises these against TVP rows).
...
- **Row copy at TVP binding**: real SQL Server's TVP semantics are pass-by-value (the proc body's modifications don't affect the caller). Since Msg 10700 already blocks all body-side modifications, pass-by-reference would be observably equivalent — but the simulator does a row-copy for clarity (and to match the documented semantics if Msg 10700 enforcement ever moves to CREATE-time validation). The per-row insert routes through `Simulation.InsertTableValuedParameterRow`, which enforces the table type's NOT NULL / CHECK / PK / UNIQUE constraints (shared with the TDS-wire TVP path).
