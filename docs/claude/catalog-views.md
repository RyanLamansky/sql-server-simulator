...
- **`sys.all_objects`** shares `sys.objects`'s shape and adds the `sys.system_objects` rows, as real's union does (`BuiltInResources.SystemObjects`: every modeled catalog view and system procedure, five of the `sp_` procedures typed `X` as real types them).
