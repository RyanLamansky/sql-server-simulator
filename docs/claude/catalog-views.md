...
- **`sys.columns`** projects per-column metadata: `object_id`, `name sysname`, `column_id` (1-based), `system_type_id tinyint`, `user_type_id int`, `max_length smallint` (byte-length — `nvarchar(50)→100`, `char(5)→5`, `-1` for the MAX form, `16` for text/ntext/image LOB pointers, `256` for sysname), `precision` / `scale tinyint` (decimal/numeric carry their declared (p,s); date/time fractional types follow `(time(N): 8+N, N)` / `(datetime2(N): 19+N, N)` / `(datetimeoffset(N): 26+N, N)`; 0 for everything else), `is_nullable` / `is_identity` / `is_computed bit`, `collation_name sysname` (set only for string types), `is_sparse bit` (the column's own marker, which `ALTER COLUMN … ADD | DROP SPARSE` moves — metadata here, since the row encoder already omits a NULL from the row), and the SMO column-node filter columns `is_xml_document` / `is_column_set` / `is_dropped_ledger_column bit` (all 0), `xml_collection_id int` (the bound collection's `xml_collection_id` for an `xml(collection)` column, 0 for untyped `xml` and every non-xml column), `vector_dimensions int` / `vector_base_type_desc nvarchar(20)` (both NULL — no vector columns modeled).
  The **"Script Table as → CREATE To"** column query reads a further batch: `is_ansi_padded bit` (1 for char/varchar/nchar/nvarchar/binary/varbinary — every table is ANSI_PADDING ON — 0 for all other types including the LOB text/ntext/image), `default_object_id int` (the column's DEFAULT constraint or bound `CREATE DEFAULT` object, 0 when none), `generated_always_type tinyint` (0/1/2 for none/ROW-START/ROW-END, from the temporal period marker), `is_hidden bit` (a HIDDEN period column), and the always-NULL/0/constant `column_encryption_key_id` / `encryption_type int` / `encryption_algorithm_name sysname` (Always Encrypted unmodeled), `graph_type int` (NULL), `is_filestream` / `is_masked bit` (0), `is_rowguidcol bit` (1 for a column declared `ROWGUIDCOL` at CREATE TABLE, else 0 — `HeapColumn.IsRowGuidCol`; DacFx reads it to re-emit `IsRowGuidColumn=True`), `rule_object_id int` (the bound `CREATE RULE` object, else 0), and `ledger_view_column_type int` / `ledger_view_column_type_desc nvarchar(60)` (NULL — ledger unmodeled).
...
- **`sys.types`** projects `name sysname`, `system_type_id tinyint`, `user_type_id int`, `schema_id int`, `is_user_defined` / `is_table_type` / `is_nullable` / `is_assembly_type bit`, and `max_length smallint` / `precision` / `scale tinyint`.
  ...
  Also projects (appended after the triple) `collation_name sysname` (database collation for the character-family types + string alias types, NULL otherwise), `principal_id int` (NULL), `default_object_id int` and `rule_object_id int` (the `CREATE DEFAULT` / `CREATE RULE` object bound to an alias type, else 0; DacFx's UDDT reverse-engineering query INNER JOINs `sys.objects` on the latter).
...
- **`sys.allocation_units`** (8-column, probe-confirmed SQL Server 2025): the storage-allocation surface SSMS's Database Properties → General page joins to compute DbSize / SpaceUsed (`sys.partitions p JOIN sys.allocation_units a ON p.partition_id = a.container_id`).
  ...
  **Self-consistency contract**: `SumDataFilePages` (Σ `total_pages` over all units) is what `sys.database_files` / `sys.master_files` size the data file from (via `ComputeDataFileSizePages` = used + headroom, floored at 640) and what `FILEPROPERTY(<db>, 'SpaceUsed')` returns, so SSMS's `SpaceAvailable = DbSize − SpaceUsed` (data-file size − Σ `total_pages`) is always ≥ 0.
...
- **`sys.all_objects`** shares `sys.objects`'s shape and adds the `sys.system_objects` rows, as real's union does (`BuiltInResources.SystemObjects`: every modeled catalog view and system procedure, five of the `sp_` procedures typed `X` as real types them, `fn_helpcollations` typed `IF`).
  The master-only `spt_values`, a `dbo` view in master on real, isn't listed, since the list is the same for every database.
  `OBJECT_NAME` / `OBJECT_SCHEMA_NAME` / `OBJECTPROPERTY` read a system object's negative id, and `OBJECT_ID` resolves a system procedure under any schema qualifier (`sp_help`, `sys.sp_help`, `dbo.sp_help`) — though not yet through an `'X'` type filter (probed 2026-09-24).
...
- **Declared column types** follow real's `sys.all_columns` for every modeled catalog view, name columns included (a `sysname` in some views, an `nvarchar(128)` in others — INFORMATION_SCHEMA's catalog and schema columns among the latter), and `CatalogView.Conform` settles a generator's value on its column's declaration (checked 2026-09-26 against SQL Server 2025).
  The columns real lists that answer alike for everything the simulator holds — `sys.tables` / `sys.views` / `sys.procedures` / `sys.sequences` replication, CDC and ledger flags, `sys.tables` retention (`-1` / `INFINITE`), `sys.parameters` encryption columns — are declared as constants (`BuiltInResources.RealConstantColumns`), so selecting one isn't a Msg 207.
  `sys.identity_columns` / `sys.computed_columns` / `sys.masked_columns` carry `sys.columns`' row (the vector columns aside) ahead of their own, built on the same per-column row (`EnumerateColumnRows`).
  Still missing: the less-used columns of `sys.dm_tran_locks` and `sys.dm_os_waiting_tasks`.
  A `sysname` in a collation other than the server default — the Service Broker names, `sys.assembly_types.assembly_class`, `encryption_algorithm_name` — is declared `nvarchar(128)` in that collation here, since the simulator's `sysname` carries one collation; `sys.external_models.parameters` is `nvarchar(max)` where real's is `json`.
- **`sys.all_views`** / **`sys.all_columns`** add **`sys.system_views`** / **`sys.system_columns`**: every modeled catalog view, `is_ms_shipped` 1, and its declared columns, each in its type's own collation rather than the database's (a `sysname` column in the server's default, a `char(2)` `type` in `Latin1_General_CI_AS_KS_WS`), while `sys.views` / `sys.columns` list neither (probed 2026-09-26 against SQL Server 2025).
- **`sys.all_views`** / **`sys.all_sql_modules`** share `sys.views` / `sys.sql_modules`'s shape and row generator — user-object parity, same precedent as `sys.all_objects`.
  ...
  The simulator ships no system modules with stored T-SQL, so `sys.all_sql_modules` is just the user modules.
...
- **`sys.computed_columns`** / **`sys.identity_columns`** carry real's full shape: `sys.columns`' row plus their own columns (SMO's column-scripting query `LEFT JOIN`s both).
  ...
  A view's or table-valued function's column that passes an identity column straight through — `Selection.ColumnIdentitySources`, which follows it through aliases, derived tables, `TOP`, `GROUP BY`, `DISTINCT`, `APPLY` and other views but drops it at an expression, a join or a set operation — is listed too, with NULL `seed_value` / `increment_value` / `last_value`, and reports `is_identity` in `sys.columns`; `IDENT_SEED` / `IDENT_INCR` / `IDENT_CURRENT` and `sp_help` over the view answer from the base column (probed 2026-09-26 against SQL Server 2025).
...
- **DacFx / `sqlpackage /Action:Export` empty views** (full probe-confirmed SQL Server 2025 shape, zero rows — the accepted-but-empty precedent): the bacpac-export reverse-engineering references 38 catalog views for genuinely-unmodeled features; each is modeled empty so a reference resolves rather than raising Msg 208.
  **All 38 exist on real SQL Server 2025** (none skipped).
  ...
  - **Partitioning** (`BuiltInResources.Indexes.cs`): `sys.partition_functions` / `partition_schemes` / `partition_range_values` / `partition_parameters` / `destination_data_spaces`.
  ...
  - **Encryption / key management / Always Encrypted / RLS / audits** (`BuiltInResources.Security.cs`): `sys.symmetric_keys` / `cryptographic_providers` (server-scope) / `crypt_properties` / `key_encryptions` / `column_master_keys` / `column_encryption_key_values` / `database_credentials` / `database_scoped_credentials` / `security_policies` / `security_predicates` / `server_audits` (server-scope) / `server_file_audits` (server-scope — was **not** previously modeled).
  ...
  - **Index / search feature views** (`BuiltInResources.Indexes.cs` + `BuiltInResources.FullTextXmlSpatial.cs`): `sys.json_index_paths` / `selective_xml_index_namespaces` / `vector_indexes` (JSON indexes, selective XML index namespaces, DiskANN vector indexes) in Indexes; `sys.registered_search_properties` in FullTextXmlSpatial.
...
- **`sys.dm_exec_connections`** projects one row per live connection from the `Network.ConnectionTransport` each session rides (probed 2026-09-25 against SQL Server 2025).
  A TDS-endpoint session reports `TCP`, both endpoints (an IPv4-mapped address in its IPv4 form), LOGIN7's TDS version as `protocol_version`, the negotiated packet size and the packets counted so far, with `encrypt_option` `TRUE` because the endpoint requires TLS.
  An in-process connection has no network side, so it takes real's shared-memory shape: `Shared memory`, `<local machine>`, endpoint 2, unencrypted, and NULL for every packet and TCP column.
  A MARS connection is one row, where real adds a `Session` row per logical session.
- **`sys.dm_exec_requests`** lists a request for the querying session (`running`) and for every other session mid-statement (`runnable`) or waiting (`suspended`), read off `SessionToken` (probed 2026-09-25 against SQL Server 2025).
  `command` is the statement being dispatched — a nested body's own statement while it runs — and `start_time` the command's start; a lock wait reports `LCK_M_<mode>` with its blocker, resource and elapsed `wait_time`, and a `WAITFOR` reports `WAITFOR`.
  The session-option columns read as `sys.dm_exec_sessions` reads them.
  `sql_handle` hashes the command's text into real's 44-byte shape (equal texts share a handle, the bytes don't match real's) and `statement_start_offset` is the running top-level statement's byte offset; `statement_end_offset` is -1, "to the end of the batch", where real reports the statement's exclusive end — which it computes 2 bytes short under a `CROSS APPLY` (probed 2026-09-25), a quirk not reproduced.
...
- **DMV server-state gating**: a restricted session (non-`dbo` — a mapped user or `guest`) reading a modeled DMV is gated by the `VIEW …STATE` permissions — server-scope DMVs (`dm_tran_locks` / `dm_os_waiting_tasks` / `dm_os_sys_info` / the three `dm_tran_*_transaction(s)` views / `dm_tran_version_store*` / `dm_tran_active_snapshot_database_transactions` / `dm_hadr_cluster`, and the `dm_exec_input_buffer` TVF) raise **Msg 300** and `dm_exec_connections` and the `dm_exec_sql_text` TVF **Msg 371**, database-scope DMVs (`dm_db_partition_stats` / `dm_hadr_database_replica_states`) raise **Msg 262**, `dm_exec_sessions` and `dm_exec_requests` self-filter to the own session, and `dm_os_host_info` / `fn_helpcollations` / `dm_db_xtp_table_memory_stats` stay public.
...
- **`sys.sql_modules`** projects one row per programmable module (procedure / view / DML + DDL trigger / scalar / inline / multi-statement function), keyed by `object_id`: `definition nvarchar(max)` (the creating batch verbatim — see the OBJECT_DEFINITION scalar below; NULL for WITH ENCRYPTION).
  **Wire-typing note**: the `definition` column is declared with a length-0 `.Type` plus a `MaxLength` of `MaxLengthSentinel`, so an expression that *references* it — SMO reads proc bodies as `ISNULL(smsp.definition, ssmsp.definition)` — must recover the MAX-ness from `MaxLength`, not the length-0 `.Type`; `ResolveColumnTypeAcrossSources` folds `MaxLength == MaxLengthSentinel` into `SqlType.AsMaxVariant` so the expression types `nvarchar(max)` and streams as PLP.
  ...
  Remaining columns: **`uses_ansi_nulls bit`** and **`uses_quoted_identifier bit`** (the module's creation-time SET-option captures — see [Creation-time SET-option capture](#creation-time-set-option-capture)), `is_schema_bound bit` (1 for a `WITH SCHEMABINDING` view or function — read from `View.IsSchemaBound` / `UserDefinedFunction.IsSchemaBound`; 0 otherwise), `null_on_null_input bit` (1 only for a scalar function declared `WITH RETURNS NULL ON NULL INPUT`), **`execute_as_principal_id int`** and the **`inline_type` / `is_inlineable`** pair (their own sections below), and `uses_database_collation bit` (1 for every schema-bound module, whatever it reads — probed 2026-09-26), and `is_recompiled` / `uses_native_compilation bit` (0).
...
- **`INFORMATION_SCHEMA.ROUTINES`** carries real's 51 columns (probed 2026-09-26 against SQL Server 2025).
  The `SPECIFIC_*` triple mirrors `ROUTINE_*`, since a T-SQL routine has no overloading.
  A scalar function's return type fills `DATA_TYPE` and the facet columns as `INFORMATION_SCHEMA.PARAMETERS` describes a parameter; a table-valued function's `DATA_TYPE` is `'TABLE'` with no facets, a procedure's NULL.
  `ROUTINE_DEFINITION` is `OBJECT_DEFINITION`'s text truncated to 4000 characters (NULL under `WITH ENCRYPTION`).
  Real's constants: a procedure `MODIFIES` SQL data with `MAX_DYNAMIC_RESULT_SETS` -1 and a NULL `IS_NULL_CALL`, a function `READS` with 0; `IS_DETERMINISTIC` is `OBJECTPROPERTY`'s `IsDeterministic`, so `'NO'` for every procedure.
...
- **`INFORMATION_SCHEMA.PARAMETERS`** (real's full 29 columns, probed 2026-09-26): one row per procedure / function parameter plus a scalar function's return value as row 0 (empty name, `OUT`, `IS_RESULT` `YES`); the type columns follow `INFORMATION_SCHEMA.COLUMNS`, the collation is the database's for a character type, and `USER_DEFINED_TYPE_*` names an alias parameter's alias or a table-valued parameter's table type (whose `DATA_TYPE` reads `table type`).
  ...
  `CHARACTER_MAXIMUM_LENGTH` covers the **binary family as well as the string one** — a `varbinary(20)` parameter reports 20 — carries the MAX sentinel `-1` for MAX-declared types and for `xml`, the documented sentinels for the legacy LOB types, and NULL for everything else (probed 2026-07-30).
...
- **The query-defined ISO views** — `CHECK_CONSTRAINTS`, `VIEW_TABLE_USAGE`, `VIEW_COLUMN_USAGE`, `ROUTINE_COLUMNS`, `SEQUENCES`, `COLUMN_DOMAIN_USAGE` (`BuiltInResources.InformationSchemaUsage.cs`, plus `ROUTINE_COLUMNS` beside `COLUMNS`).
  Real defines each as a plain query over `sys.*`, and each generator follows that definition as `OBJECT_DEFINITION` returns it (read 2026-09-26 from SQL Server 2025): the two view-usage views read `sys.sql_dependencies` (the table form distinct per object, the column form one row per referenced column), `ROUTINE_COLUMNS` is `COLUMNS` over a table-valued function's returned columns, `SEQUENCES`' `DECLARED_*` comes from the declared type's own `sys.types` row, and `COLUMN_DOMAIN_USAGE` lists every alias-typed column `sys.columns` holds.
...
- **`sys.databases`** (full **98-column** projection matching SQL Server 2025; one row per `Database` via `DbId.DatabasesWithIds`, ordered by `database_id`).
  ...
  **`database_id` is `int`** (SMO/consumers expect int) — distinct from the `DB_ID()` scalar, which still returns `smallint`.
  ...
  `is_query_store_on` follows `Database.QueryStore.DesiredState` — any state but OFF reads on here, READ_ONLY included, since the flag tracks the *desired* state — so it always agrees with the `sys.database_query_store_options` row it is read beside.
...
- **`sys.master_files`** (32-column, **one data file (`type` 0, ROWS) + one log file (`type` 1, LOG) per database** via `DbId.DatabasesWithIds`, join key `database_id`).
  ...
  File contents are synthetic: logical name `<db>` / `<db>_log`, physical path `/var/opt/mssql/data/<db>.mdf` / `_log.ldf`, `state` 0/ONLINE, and a page-denominated `max_size` / `growth` pair — real measures both in 8 KB pages whenever `is_percent_growth` is 0, so the 64 MB default growth is **8192**, not 65536, and `max_size` is -1 (unlimited) on the data file but the 2 TB ceiling **268435456** on the log (probe-confirmed).
...
- **`OBJECT_ID` resolves catalog views**: `OBJECT_ID('sys.<view>')` / `OBJECT_ID('[sys].[<view>]')` (and the `'V'`-typed form) return a non-NULL `int` for any registered `sys.*` / `INFORMATION_SCHEMA.*` catalog view, via `BatchContext.TryResolveCatalogView`.
  ...
  A modeled system procedure's id is real's too (`CatalogViewObjectIds.ByProcedureName`, 2026-09-24).
...
- **`sp_datatype_info_100`** system procedure (`Simulation.DatatypeInfo.cs`, dispatched like `xp_msver` / `sp_tablecollations_100`): the ODBC `SQLGetTypeInfo` backing proc a driver (ODBC Driver 18, JDBC, any `SQLGetTypeInfo` caller) queries on connect to learn each type's precision/scale — without it the driver bound temporal parameters at scale 0, dropping a `datetime2` parameter's sub-second fraction before it was even sent.
  ...
  `@data_type int = 0` (positional or named, absent → 0; an explicit NULL matches nothing, since real's range test compares against it — probed 2026-09-25) selects a single `DATA_TYPE` when non-zero or every type when 0; `@ODBCVer tinyint = 2` (positional or named, absent → 2) collapses to 2 (values < 3) or 3, choosing a version-tagged row set (37 rows each).
  ...
  The older **`sp_datatype_info`**, which pre-2008 drivers call, serves its own 37-row sets captured from real in real's order: its backward-compatible columns report `date` / `time` / `datetime2` / `datetimeoffset` as `nvarchar` (-9) and `xml` as `ntext` (-10).
- **`sp_tables` / `sp_columns_100`** system procedures (`Simulation.CatalogProcs.cs`, dispatched like `sp_datatype_info_100`): the ODBC `SQLTables` / `SQLColumns` backing procs JDBC's `DatabaseMetaData.getTables` / `getColumns` (Hibernate schema validation, generic tooling) call on connect to enumerate the catalog.
  ...
  - **`sp_tables`** yields the five ODBC columns `TABLE_QUALIFIER` / `TABLE_OWNER` / `TABLE_NAME sysname`, `TABLE_TYPE varchar(32)` (`'TABLE'` | `'VIEW'`), `REMARKS varchar(254)` (always NULL), one row per table/view — the catalog views included, as views of `sys` / `INFORMATION_SCHEMA`, whose columns `sp_columns` lists too (probed 2026-09-26 against SQL Server 2025) — sorted by (`TABLE_TYPE`, `TABLE_QUALIFIER`, `TABLE_OWNER`, `TABLE_NAME`).
  ...
  - **`sp_columns_100`** yields the 29-column ODBC `SQLColumns` set, one row per column of every matching table/view in `ORDINAL_POSITION` order.
    ...
    A `geography` / `geometry` / `hierarchyid` column reads as `SQL_SS_UDT` (-151) with `SS_UDT_CATALOG_NAME` the database, `SS_UDT_SCHEMA_NAME` `sys` and the `Microsoft.SqlServer.Types` assembly-qualified name, and a `sysname` column keeps `sysname` as its `TYPE_NAME` (probed 2026-09-25).
  - The older **`sp_columns`** returns the first 18 of those columns plus `SS_DATA_TYPE`, reporting the newer types the way pre-2008 drivers expect: `date` / `time` / `datetime2` / `datetimeoffset` as `nvarchar` (-9) of their literal's length, `xml` and `nvarchar(max)` as `ntext`, `varchar(max)` as `text`, `varbinary(max)` as `image`, and the CLR types `image`-coded under their own names; it takes no `@fUsePattern`, and any `@ODBCVer` but 3 reads as 2 (probed 2026-09-25).
...
- **`sp_pkeys` / `sp_fkeys` / `sp_statistics_100` / `sp_stored_procedures`** system procedures (`Simulation.CatalogProcs.cs`, dispatched like `sp_tables`): the ODBC `SQLPrimaryKeys` / `SQLStatistics` / `SQLProcedures` backing procs JDBC's `DatabaseMetaData.getPrimaryKeys` / `getIndexInfo` / `getProcedures` call to enumerate the live catalog's keys, indexes, and stored procedures.
  All result-set shapes and per-column values are probe-confirmed against SQL Server 2025.
  ...
  - **`sp_fkeys`** (`Simulation.CatalogProcs.ForeignKeys.cs`) yields the 14-column ODBC `SQLForeignKeys` set, one row per column of each foreign key between the named tables — the proc JDBC's `getImportedKeys` / `getExportedKeys` and pyodbc's `foreignKeys` call.
    Its quirks are real's own (probed 2026-09-25): given a primary-key table the rows sort by foreign-key table then `KEY_SEQ` and the rules swap `sys.foreign_keys`' CASCADE / NO ACTION codes (0 / 1) while passing SET NULL (2) and SET DEFAULT (3) through, but given only a foreign-key table they sort by primary-key table and a rule reads 0 for CASCADE and 1 for anything else.
    Names are exact, a missing or empty owner means the default schema, `PK_NAME` is the referenced key or unique index, and neither table name is Msg 15252.
  - **`sp_server_info`** (`Simulation.CatalogProcs.Server.cs`) yields ODBC's `SQLGetInfo` attribute rows (`attribute_id int`, `attribute_name varchar(60)`, `attribute_value varchar(255)`) byte-identical to real's: the `sys.spt_server_info` stock plus `IDENTIFIER_CASE` and `COLLATION_SEQ` derived from the server collation; `@attribute_id` narrows to one.
  - **`sp_databases`** lists every database the session can open (`HAS_DBACCESS`) with `DATABASE_SIZE` the sum of its `sys.master_files` pages in KB, ordered by name.
  - **`sp_statistics_100`** — and `sp_statistics`, which real answers identically for rowstore indexes — yields the 13-column ODBC `SQLStatistics` set: a table-cardinality **summary row first** (`TYPE` = 0 `SQL_TABLE_STAT`, every index column NULL), then one row per index key column.
    ...
    Rows sort by (`NON_UNIQUE`, `TYPE`, `INDEX_NAME`, `SEQ_IN_INDEX`) after the summary; `@table_name` is an exact identifier, `@index_name` is a **LIKE pattern** over the index rows defaulting to `'%'` (so an omitted or `DEFAULT` argument lists every index, and only an explicit NULL yields the summary row alone — probed 2026-09-25), `@is_unique = 'Y'` restricts to unique indexes (the summary row is always emitted), `@accuracy` / `@ODBCVer` are accepted and ignored.
    Indexes are enumerated through `HeapTable.IndexIdentities()` — the same single index-id authority `sys.indexes` reads.
  - **`sp_stored_procedures`** yields the eight-column ODBC `SQLProcedures` set, one row per user stored procedure (`name;1`) and function (`name;0`, still `PROCEDURE_TYPE` 2 — probed 2026-09-26 against SQL Server 2025): `PROCEDURE_QUALIFIER` / `PROCEDURE_OWNER` (`sysname`), `PROCEDURE_NAME` (`nvarchar(134)`, carrying the trailing `;1` group number), `NUM_INPUT_PARAMS` / `NUM_OUTPUT_PARAMS` / `NUM_RESULT_SETS` (`int`, always `-1` — real doesn't compute them), `REMARKS` (`varchar(254)`, NULL), `PROCEDURE_TYPE` (`smallint`, 2 = `SQL_PT_PROCEDURE`).
    `@sp_name` / `@sp_owner` are LIKE patterns; rows sort by (`PROCEDURE_OWNER`, `PROCEDURE_NAME`).
    The simulator has no system-procedure catalog, so — unlike real, which also lists the ~1600 `sys` procs and the `sys` table-valued functions — only user routines are projected.
  - **`sp_sproc_columns`** / **`sp_sproc_columns_100`** (`Simulation.SprocColumns.cs`) yield ODBC's `SQLProcedureColumns` set for every procedure (`name;1`) or function (`name;0`) the patterns match: `@RETURN_VALUE` at ordinal 0 (a procedure's non-nullable `int`, a scalar function's return type), or a table-valued function's single `@TABLE_RETURN_VALUE` result row, then its parameters — each nullable, no `COLUMN_DEF`, `COLUMN_TYPE` 1 / 2 for input / output, a TVP as type -153 named after its table type.
    The type cells are `sp_columns`' own for the same type in its classic or `_100` spelling, except that a classic temporal parameter keeps its fractional scale and a datetime subcode `sp_columns` blanks; `@fUsePattern = 0` matches names exactly (probed 2026-09-26 against SQL Server 2025).
    Like `sp_stored_procedures` it lists user routines only, where real's no-argument call also lists the system ones.
  - **`sp_special_columns`** / **`sp_special_columns_100`** (`Simulation.SpecialColumns.cs`) answer ODBC's `SQLSpecialColumns` by following real's own procedure text, quirks included: the index `@nullable = 'O'` settles on, SQL Server 2025's doubled `vector` rows under `@ODBCVer = 3`, and the classic procedure's included columns are each described on the method (probed 2026-09-26 against SQL Server 2025).
  - **`sp_table_privileges`** / **`sp_column_privileges`** (`Simulation.PrivilegeProcs.cs`) answer `SQLTablePrivileges` / `SQLColumnPrivileges` from the owner's implicit privileges plus the standing object and column GRANTs.
    `sp_table_privileges '%'` also lists the catalog views, owned by their schema, but not every `sys` object real carries.
  All reachable unqualified or `sys.`-qualified from any current database.
- **`sp_recompile @objname`** answers the class-0 **Msg 15070** (`Object '<objname>' was successfully marked for recompilation.`) for anything in the schema namespace and **Msg 15165** otherwise, echoing the argument as passed (probed 2026-09-25); there is nothing further to mark, the plan cache keying on the schema version.
- **`sp_rename`** system procedure (`Simulation.Rename.cs`, dispatched like `sp_tables`): the object / column / index rename schema-migration tools (Alembic's `rename_table` / `alter_column`, SSMS, EF Core's `RenameSequence`) emit.
...
- **The `sp_help` family** (`Simulation.HelpProcs.cs` + `Simulation.HelpProcs.SpHelp.cs`, dispatched like `sp_tables`): `sp_helptext`, `sp_help`, `sp_helpindex`, `sp_helpconstraint` — the formatted-metadata procs interactive sessions and scripting fall back on.
  ...
  - **`sp_help [@objname]`** emits a sequence of result sets whose membership depends on what the name resolved to.
    ...
    **An object name**: `Name` / `Owner sysname` / `Type nvarchar(31)` / `Created_datetime datetime`; then, when the object has columns, the ten-column detail set `Column_name` / `Type sysname` / `Computed` / `Length int` / `Prec` / `Scale varchar(5)` / `Nullable` / `TrimTrailingBlanks` / `FixedLenNullInSource varchar(35)` / `Collation sysname`; then — for type `S ` / `U ` / `V ` / `TF` only, so an *inline* TVF is excluded — the `Identity` / `Seed` / `Increment numeric(38,0)` / `Not For Replication int` set and the `RowGuidCol` set; then, when the object has parameters, `Parameter_name` / `Type sysname` / `Length smallint` / `Prec` / `Scale` / `Param_order int` / `Collation sysname` (a scalar function leads with its return value as an empty-named row at `Param_order` 0).
    A **table** additionally gets `Data_located_on_filegroup` (always `PRIMARY`), `sp_helpindex`'s set, `sp_helpconstraint @nomsg = 'nomsg'`'s sets, and a `Table is referenced by views` set listing every schema-bound view over it, indexed or not (or severity-10 **Msg 15647**).
    A **view** gets the Msg 15469 / 15470 pair real prints unconditionally, then its own index set.
    A **catalog view** (`sp_help 'sys.objects'`) is described as a view owned by its schema, with the identity / rowguidcol pair and the Msg 15469 / 15470 / 15472 trio (probed 2026-09-25); its `Created_datetime` is the simulator's fixed system-object date where real reports its resource-database build date.
    The three procedures run under `NOCOUNT`, so no set reports a row count — as do `sp_helptext`, `sp_helpdb`, `sp_helpfile`, `sp_helpstats`, `sp_helpuser`, `sp_spaceused`, `sp_configure`, `sp_databases`, `sp_depends`, `sp_fkeys` and `sp_statistics_100`, while `sp_tables` / `sp_columns` / `sp_pkeys` and the rest of the ODBC catalog family keep their counts (the dispatch wraps each NOCOUNT one in `Uncounted`) — and their messages arrive **among** the sets as real sends them (probed 2026-09-26): a blank `PRINT` (one space) ahead of each section at the line of real's own source that prints it, and the "nothing to report" messages attributed to the procedure by the name it was called by — `sp_help`'s calls are to `sys.sp_helpindex` / `sys.sp_helpconstraint`, so its table output carries those names; `HelpMessage` in `Simulation.HelpProcs.cs` holds the lines.
...
- **`sp_helpdb`** (`Simulation.HelpProcs.Database.cs`, dispatched like `sp_tables`): the database list, or one database's detail plus its file allocation.
  ...
  A database `HAS_DBACCESS` reports 0 for is dropped from the report with the severity-10 **Msg 15622** in its place — for an ordinary login that is `model` and every database it has no user in, while a sysadmin (and the default session) lists them all (probed 2026-09-25 both ways).
  An unknown `@dbname` → **Msg 15010**.
  With one argument the summary is followed by real's single-space `PRINT` and `sp_helpfile`'s own set: `name sysname` / `fileid smallint` / `filename nchar(260)` / `filegroup sysname` / `size` / `maxsize` / `growth nvarchar(18)` / `usage varchar(9)`, over the same synthetic two-file model `sys.database_files` / `sys.master_files` project (`<db>` on PRIMARY, `<db>_log` with a NULL filegroup, unlimited max size, 64 MB growth).
...
- **`sp_MSforeachdb`** (`Simulation.ForEachDatabase.cs`, dispatched like `sp_tables`): the same shape one scope out — one to three command templates run once per accessible database, `@precommand` first and `@postcommand` last, each its own dynamic batch.
  Its parameter list carries **no `@whereand`**, so a positional `@precommand` sits one place earlier than `sp_MSforeachtable`'s; the database list therefore comes from catalog state in C# (`DbId.DatabasesWithIds` filtered by `HAS_DBACCESS`, so a sysadmin session visits every database, `model` included, and an ordinary login only those it can open) rather than from an embedded query.
...
- **Legacy SQL-Server-2000 compatibility views** (`BuiltInResources.LegacyCompat.cs`): `sysobjects` / `syscolumns` / `sysusers` and `sys.system_objects`, the surface SSMS's Database-Properties dialog reaches.
  ...
  - **`syscolumns`** (32-column, `BuiltInResources.Syscolumns.cs`) resolves the same two ways and is built over the rows `sys.columns` and `sys.parameters` project, so it inherits their coverage: table, view, table-valued function and table-type columns (`number` 0), procedure and table-valued function parameters (`number` 1), and a scalar function's parameters and return value (`number` 0, the return value a nameless row 0 flagged `isoutparam`).
    On top it carries SQL Server 2000's storage encodings, each probed 2026-09-26 against SQL Server 2025 across every system type both nullable and not: `type` / `usertype` are the legacy codes (most fixed-width types have a separate nullable `type`), `status` packs nullable 0x08 / padded string 0x10 / nullable `char` or `binary` 0x20 / computed or output 0x40 / identity 0x80, `typestat` NOT NULL and padded-string bits, and `offset` the 2000 row layout — fixed-width NOT NULL columns and every `bit` (eight to a byte) packing from 2, every other column counting -1, -2, …, computed columns and every non-table object reading 0, repacked after a `DROP COLUMN`.
    `collationid` / `tdscollation` are `COLLATIONPROPERTY`'s `CollationId` / `TDSCollation` for a string column.
...
All fourteen `sys.query_store_*` / `query_context_settings` / `database_query_store_*` views ship, registered together in `BuiltInResources.QueryStore.cs`, with column shapes probe-confirmed against SQL Server 2025 (2026-08-08).
The **configuration** is live — `ALTER DATABASE … SET QUERY_STORE` retains everything it parses, see [`database-options.md`](database-options.md#query-store) — but nothing is ever captured, which splits the views three ways.

- **`sys.database_query_store_options`** (22-column, **per-database**: the row set keys off the database *context* the view resolved to, not a `database_id` column, so a three-part `master.sys.database_query_store_options` read reports master's).
  Projects the database's retained `QueryStoreOptions`.
  **`master` and `tempdb` return zero rows** — real refuses to host a store on either — while `model`, `msdb` and every user database return one, which is the split a live 2025 instance returns.
  `actual_state` tracks `desired_state` exactly (real's two diverge only while a store is transitioning or has forced itself read-only), so `readonly_reason` stays 0 and `actual_state_additional_info nvarchar(4000)` the empty string — both what real reports for a healthy store.
  `current_storage_size_mb` is 0: nothing is stored.
  The four `capture_policy_*` columns project NULL unless the capture mode is CUSTOM, which is real's own masking.
...
- **The eleven capture views are always empty**, via the shared `EmptyCatalogRows`: `query_store_query`, `query_store_query_text`, `query_store_query_variant`, `query_store_plan`, `query_store_plan_feedback`, `query_store_plan_forcing_locations`, `query_store_query_hints`, `query_store_runtime_stats`, `query_store_runtime_stats_interval`, `query_store_wait_stats`, `query_context_settings`.
  The last two of those are the interesting ones: real projects a row in each on a freshly-enabled store, but both are capture artifacts (the current interval, one context-settings row) rather than fixed metadata, so empty is the coherent answer for a store that captures nothing.
  `sys.query_store_runtime_stats`' 81-column shape splits by metric group — the nine "core" groups (`duration`, `cpu_time`, `logical_io_reads`/`_writes`, `physical_io_reads`, `clr_time`, `dop`, `query_max_used_memory`, `rowcount`) expose NOT NULL `last`/`min`/`max` bigint columns and the four "extended" ones (`num_physical_io_reads`, `log_bytes_used`, `tempdb_space_used`, `page_server_io_reads`) expose them NULL — built by `BuildQueryStoreRuntimeStatsColumns`.
  SSMS's probe does `IF EXISTS (SELECT TOP(1) 1 FROM sys.query_store_runtime_stats)`, which falls to its ELSE.
- **Two views carry rows anyway**, because their contents are fixed metadata rather than captured data.
  `sys.query_store_replicas` projects the four replica roles (Primary / Secondary / Geo Secondary / Geo HA Secondary) for a user database and nothing for any of the four system databases — the split real reports, `model` included even though its own store is on.
  `sys.database_query_store_internal_state` projects one zeroed row for every database, `master` included.
...
Reads `SchemaObject.DefinitionText`, captured at CREATE / ALTER time by `Simulation.BuildModuleDefinition` — the whole batch the module was created in, from its first character to its last: leading whitespace and comments and a trailing `;` or comment are all kept (probed 2026-09-26 against SQL Server 2025).
...
The computed-column family (probed 2026-09-26) is NULL on a plain column except `IsIndexable`, which is 1 unless the column is a LOB or a `max` type; on a computed column `IsDeterministic` and `IsPrecise` judge the stored expression, `IsIndexable` needs it deterministic, not LOB / `max`, and persisted or precise (a persisted float expression qualifies), `IsSystemVerified` is 1 and the two data-access properties 0.
`IsFulltextIndexed` is 1 for a column of its table's full-text index, and `IsXmlIndexable` 1 for a table's `xml` column, 0 for its others and NULL on a view's.
...
Each database exposes exactly two synthetic files, mirroring `sys.database_files` / `sys.master_files`: `<db>` (`file_id` 1, primary ROWS, on PRIMARY) and `<db>_log` (`file_id` 2, LOG).
So `FILE_ID(N'simulated')` = 1, `FILE_NAME(1)` = `simulated`, and the file-level scalars stay consistent with those views.
...
Cross-cutting notes:
- **Column subset**: a few views ship the load-bearing subset of real's columns that EF / migration tooling, DacFx, SMO and the probes queried — the lock and waiting-task DMVs — so `SELECT *` returns fewer columns than real there (the complete list is the column diff against real's `sys.all_columns`).
  Each column shipped declares the type and length, the collation and the nullability real's `sp_describe_first_result_set` reports for it — a declaration giving an unsized string or binary type plus a separate length is retyped to that length when the view is built (`CatalogView.SizedColumn`) (a sweep of every catalog view, 2026-09-25), and a declared NOT NULL column never yields NULL across the AdventureWorks, WideWorldImporters and Insite bacpacs.
