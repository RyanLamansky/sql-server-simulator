...
The correlation-safety contract is narrow: **only** the catalog-view construction site sets `MaterializeOnce`; correlated / lateral sources (derived tables, APPLY, VALUES, TVFs, views) never set it and keep their correct per-outer-row execution.
...
- **`sys.schemas`** projects `name sysname`, `schema_id int`, `principal_id int NULL`.
  ...
  `principal_id` matches real (probe-confirmed against SQL Server 2025): every fixed schema is owned by the like-id principal (`principal_id = schema_id` for ids ≤ 4 or ≥ 16384), and user schemas (ids 5..16383) are owned by dbo (`principal_id = 1`).
...
- **`sys.stats`** (17-column, probe-confirmed SQL Server 2025): one row per index `sys.indexes` reports **excluding the heap** (`index_id 0` carries no statistic), with `stats_id = index_id` and `name` = the index/constraint name (real SQL Server's index-backing statistic shares the index's id and name).
  **Auto-created column statistics (`_WA_Sys_*`) aren't modeled** — the simulator has no stats lifecycle — so `auto_created` / `user_created` / `no_recompute` are always 0 and no column-only stats rows appear (SMO's `NoAutomaticRecomputation` reads `ISNULL(no_recompute, 0)` = 0).
  `stats_generation_method` = 0 / `_desc` = `'Sort based statistics'`; the filter / temporary / incremental / replica columns are 0 or NULL.
...
- **`sys.dm_exec_sessions`** projects one row per **live connection** on the server (`Simulation.Connections`, snapshotted under the registry lock).
  Session-backed columns reflect the connection's real state: `quoted_identifier` / `lock_timeout` / `transaction_isolation_level` (1/2/3/4/5 for RU/RC/RR/Serializable/Snapshot) / `context_info` / `row_count` (`@@ROWCOUNT`) / `prev_error` (`@@ERROR`) / `open_transaction_count` / `database_id`; `status` is `running` for the querying session, `sleeping` for the rest.
  The remainder are probe-confirmed fresh-session defaults (SQL Server 2025): `endpoint_id 4`, `group_id 2`, `client_version 7`, `text_size -1`, `arithabort 0`, the ANSI bits, `login_name`/`original_login_name` = the principal placeholder `SUSER_SNAME()` uses, `security_id` = sa's single-byte SID, `authenticating_database_id 1`.
...
- **`INFORMATION_SCHEMA.SCHEMATA`** (6 cols): only the schemas the simulator models (no role-principal padding — real SQL Server lists 13 schemas because of `db_owner`/`db_datareader`/etc., and we have no principal model).
  SCHEMA_OWNER mirrors SCHEMA_NAME.
...
- **`sys.sql_modules`** projects one row per programmable module (procedure / view / DML + DDL trigger / scalar / inline / multi-statement function), keyed by `object_id`: `definition nvarchar(max)` (the verbatim CREATE source — see the OBJECT_DEFINITION scalar below; NULL for WITH ENCRYPTION).
  **Wire-typing note**: the `definition` column is declared with a length-0 `.Type` plus a `MaxLength` of `MaxLengthSentinel`, so an expression that *references* it — SMO reads proc bodies as `ISNULL(smsp.definition, ssmsp.definition)` — must recover the MAX-ness from `MaxLength`, not the length-0 `.Type`; `ResolveColumnTypeAcrossSources` folds `MaxLength == MaxLengthSentinel` into `SqlType.AsMaxVariant` so the expression types `nvarchar(max)` and streams as PLP.
  ...
  Remaining columns: `uses_ansi_nulls bit` (constant 1), **`uses_quoted_identifier bit`** (the module's creation-time `QUOTED_IDENTIFIER` capture off `SchemaObject.UsesQuotedIdentifier` — 0 for a module created under `SET QUOTED_IDENTIFIER OFF`, re-stamped by `ALTER` / `CREATE OR ALTER`; see [`grammar.md`](grammar.md#per-object-creation-time-capture)), `is_schema_bound bit` (1 for a `WITH SCHEMABINDING` view or function — read from `View.IsSchemaBound` / `UserDefinedFunction.IsSchemaBound`; 0 otherwise), `uses_database_collation` / `is_recompiled` / `uses_native_compilation` / `inline_type` / `is_inlineable bit` (all 0 — placeholder constants), `null_on_null_input bit` (1 only for a scalar function declared `WITH RETURNS NULL ON NULL INPUT`), `execute_as_principal_id int` (always NULL — no EXECUTE AS principal modeled).
...
- **`sys.databases`** (full **98-column** projection matching SQL Server 2025; one row per `Database` via `DbId.DatabasesWithIds`, ordered by `database_id`).
  ...
  **`database_id` is `int`** (SMO/consumers expect int) — distinct from the `DB_ID()` scalar, which still returns `smallint`.
  ...
  `recovery_model` is `1`/`FULL` for the `model` template and `3`/`SIMPLE` for every other database (mirroring the reference instance).
...
- **`sys.master_files`** (32-column, **one data file (`type` 0, ROWS) + one log file (`type` 1, LOG) per database** via `DbId.DatabasesWithIds`, join key `database_id`).
  ...
  File contents are synthetic: logical name `<db>_Data` / `<db>_Log`, physical path `/var/opt/mssql/data/<db>.mdf` / `_log.ldf`, `state` 0/ONLINE, `max_size` -1 (unlimited), `growth` 65536 (64 MB), all `is_*` flags 0.
...
- **`sp_who` / `sp_who2`** (`Simulation.SessionProcs.cs`, dispatched like `sp_tables`): the session lists, one row per open `SimulatedDbConnection` in the live registry, sorted by spid.
  ...
  - **Divergences.** `ecid` is 0 (no parallel worker threads), `request_id` is 0, and `hostname` / `ProgramName` are empty — no client host or program name reaches a session.
...
**`IsQuotedIdentOn` / `ExecIsQuotedIdentOn`** both read the object's creation-time `QUOTED_IDENTIFIER` capture (`SchemaObject.UsesQuotedIdentifier`, the same source as `sys.sql_modules.uses_quoted_identifier`) and agree on every module; they diverge on a **table**, which the shorter spelling answers 1 for regardless of the creating session while the module-only `ExecIs…` form answers NULL.
A sequence, synonym or key constraint answers NULL to both.
Capture semantics — including that the module's *body* runs under it — are in [`grammar.md`](grammar.md#per-object-creation-time-capture).
`ExecIsAnsiNullsOn` remains a constant 1 for any module (the ANSI_NULLS half of the pair isn't captured yet).
...
- **`COLUMN_DEFAULT` always NULL** (fidelity gap): real SQL Server renders default expressions as parenthesized text (`(sysdatetime())`).
  Serializing arbitrary `Expression`s back to SQL is a separate (sizable) bundle; the column ships as NULL until that lands.
