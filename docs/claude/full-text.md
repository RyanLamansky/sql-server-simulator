...
DDL + catalog views + per-table index slot ship; query-time predicates (`CONTAINS` / `FREETEXT` / `CONTAINSTABLE` / `FREETEXTTABLE` / `SEMANTIC*`) raise explicit `NotSupportedException`.
...
The simulator reports Full-Text as installed (`SERVERPROPERTY('IsFullTextInstalled') = 1`; CREATE FULLTEXT CATALOG / INDEX are modeled), so for self-consistency `FULLTEXTSERVICEPROPERTY('IsFullTextInstalled')` returns `1`.
