...
| 8 | Deferred computed columns (only for tables phase 2 fell back on — a forward UDF reference; these append at the end, so their `sys.columns.column_id` lands after the simple columns rather than at the model ordinal) + indexes (order matters: computed cols before filtered indexes that reference them) + **columnstore indexes** (`CREATE … COLUMNSTORE INDEX`, a clustered one without the column list DacFx writes for it) + **XML indexes** (`CREATE [PRIMARY] XML INDEX`; primaries precede secondaries in DACFx's name-sorted document order, so a secondary's `USING XML INDEX` reference resolves) + the **full-text-index retries** phase 5 deferred |
