...
The segment-array (`int[][]`) form is a transient decode used only by `ToString()` / the instance methods.
...
- `SqlValue.FromHierarchyIdBytes(byte[])` — stores raw bytes verbatim (BACPAC import, ADO.NET byte parameters, the reverse CAST after it validates).
  **Opaque passthrough**: bytes in an unmodeled tier still round-trip through storage/export even though `ToString()` can't decode them.
...
`HierarchyIdOrdPath.Encode(int[][])` / `.Decode(bytes)` are the single canonical codec.
