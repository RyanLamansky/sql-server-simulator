...
- **A SERIALIZABLE writer's fence past the single-table forms** — a SERIALIZABLE `UPDATE` / `DELETE` fences its WHERE's interval in `RangeX-X` (see [`locking.md`](locking.md#what-a-serializable-writer-takes)), but the joined forms, `MERGE`, and a `WITH (HOLDLOCK)` / `SERIALIZABLE` hint on the target still take only table-IX plus row-X.
...
- **Query Store capture and its procedures** — the configuration ships and every catalog view resolves ([`database-options.md`](database-options.md#query-store), [`catalog-views.md`](catalog-views.md#query-store)), but nothing is ever captured, so a database can report its store READ_WRITE with eleven empty capture views behind it — a state real never reaches.
  Capture wants a per-database store of query texts / plans / runtime intervals fed from the execution path, plus the eleven `sp_query_store_*` procedures that act on it (flush, force / unforce plan, remove query / plan, reset exec stats, set / clear hints, consistency check).
  Real's runtime validation of a sub-option value is unbuilt with it — Msg 153 for `DATA_FLUSH_INTERVAL_SECONDS = 0` and for an `INTERVAL_LENGTH_MINUTES` outside {1, 5, 10, 15, 30, 60, 1440}, at *different* class/state pairs (15/5 and 16/6), while `MAX_STORAGE_SIZE_MB = 0` and `STALE_QUERY_THRESHOLD_DAYS = 0` are accepted; the patchiness is why a partial rule would over-raise.
...
- **Browse mode — `CommandBehavior.KeyInfo`, `SET NO_BROWSETABLE ON`, `FOR BROWSE`** — SqlClient's `KeyInfo` (and so `DataAdapter.FillSchema`, `MissingSchemaAction.AddWithKey`, `SqlCommandBuilder`) wraps the query in `SET NO_BROWSETABLE ON / OFF`, and real then appends each base table's key columns and rowversion as **hidden** trailing columns carried in every row, and follows COLMETADATA with a `TABNAME` token (the base tables) and a `COLINFO` token (per column its table number and a status of EXPRESSION `0x04` / KEY `0x08` / HIDDEN `0x10` / DIFFERENT_NAME `0x20` plus the base name), which `GetSchemaTable` turns into `BaseTableName` / `BaseColumnName` / `IsKey` / `IsExpression` / `IsAliased` (captured 2026-09-26 for `select name, amt from w1`: hidden `id` with status `0x18`, hidden `rv` with `0x10`).
  The simulator discards `NO_BROWSETABLE`, so those properties read empty; the work is appending the hidden projections so their values ride the rows, then the two tokens.
...
- **System stored procedures** (`sp_*` family) — formatted-metadata / management procs invoked via `EXEC sp_name`.
  Shipped so far: the `sp_help` family (`sp_help` / `sp_helptext` / `sp_helpindex` / `sp_helpconstraint` / `sp_helpdb` / `sp_helpfile` / `sp_helpstats` / `sp_helprotect` / `sp_helptrigger` / `sp_helpuser`), `sp_depends`, the ODBC/JDBC catalog set (`sp_tables` / `sp_columns` / `sp_columns_100` / `sp_pkeys` / `sp_fkeys` / `sp_statistics` / `sp_statistics_100` / `sp_stored_procedures` / `sp_datatype_info` / `sp_datatype_info_100` / `sp_server_info` / `sp_databases`), `sp_spaceused`, `sp_who` / `sp_who2`, `sp_MSforeachtable` / `sp_MSforeachdb`, `sp_rename`, `sp_configure` and the `sp_xml_preparedocument` / `sp_xml_removedocument` pair — see [`catalog-views.md`](catalog-views.md).
  Still unregistered → **Msg 2812** ("Could not find stored procedure '…'."): the rest of the ODBC catalog set — `sp_special_columns`, `sp_column_privileges`, `sp_table_privileges` (the last two read real's hidden `spt_table_privileges_view` / `spt_column_privileges_view`) — `sp_MSforeach_worker` (the two `sp_MSforeach*` procs materialize their name lists rather than driving the global cursor it consumes), the `sp_add*` management family.
...
- **Unassigned code points and lone surrogates weigh nothing on real, and something here** — real treats a code point its collation version has no weight for as ignorable, so `N'a' + NCHAR(0x0378) = N'a'` is true under every non-binary collation probed (the SQL collation, `Latin1_General_CI_AS`, `Latin1_General_100_CI_AS` and its `_SC` form), and `NCHAR(0xFFFF)` behaves the same under `_100`; a lone surrogate half is ignorable under the SQL collation and the version-80 `Latin1_General_CI_AS` but carries a weight under `_100` and `_100_SC` (probed 2026-09-25 against SQL Server 2025).
  The simulator's `CompareInfo` weights all of them, so each of those equalities is false here, and a search for one follows: `CHARINDEX`, `REPLACE` and `STRING_SPLIT` find an unassigned or lone-half needle where real's weightless-needle rule reports not-found (`CHARINDEX(NCHAR(55357), N'a' + NCHAR(55357) + N'b')` is 0 on real, 2 here, under the SQL collation).
  Non-`_SC` `TRANSLATE` over a pair's halves inherits the same rule — real answers `xZZ` for `TRANSLATE(N'x' + <pair>, <pair>, N'ZQ')` and leaves the pair alone for a list of `N'a'` + the low half, where the simulator maps each half by its own position.
  Closing it wants a per-collation-version table of which code points carry no weight, applied to comparison, sort keys and search alike; the mirror image — a character real weights and `CompareInfo` ignores — is in the [over-permissive register](#over-permissive-register).
