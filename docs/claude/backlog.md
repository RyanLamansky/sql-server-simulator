...
- **A function call left unclosed at end of batch is accepted** — `SELECT abs(-1` answers `1` here and is **Msg 102** `near '1'` on real, as `SELECT datalength(cast(N'ab' as varchar(10))` is Msg 102 `near ')'` (probed 2026-08-05).
  The argument-list parse treats end of input as a closing paren rather than requiring one; a *nested* call's missing paren is caught by whatever token follows, so only the batch-final one leaks.
...
- **A bare column is legal against a GROUP BY *expression* that contains it, and Msg 8120 names the wrong column** — `SELECT a FROM t GROUP BY a + b` runs here where real raises **Msg 8120** (grouping by an expression doesn't put its operands in scope), and where both engines raise, real names the *first* ungrouped select-list column (`t.a`) while the simulator names a later one (`t.n`) — probed 2026-08-05.
  The containment check credits a column against any grouping expression that mentions it; real credits only an expression the projection repeats verbatim.
- **Msg 4145 names the empty string at end of input** — `SELECT … WHERE a` and `WHERE (a) COLLATE X` report `near ''` where real names the last token it read (`near 'a'` / `near 'Latin1_General_CI_AS'`) — probed 2026-08-05.
  General to the factory rather than to any predicate shape: the token is null there, so the Msg 102-style "last consumed token" fallback the naming rule wants isn't threaded.
- **Two DML sites still report Msg 207 where real reports Msg 4104** — the general classification ships (see [`schemas.md`](schemas.md#which-error-an-unresolved-name-reports)), but a SET list's **left-hand side** (`UPDATE t SET zz.id = 5`) drops its qualifier at the parse, so a bogus one is silently accepted, and an `INSERT … VALUES (zz.id)` expression reports 207.
  The SET-LHS half wants the qualifier carried alongside the column name through `rawAssignments`, which the joined and join-view write paths share.
...
- **`SET ROWCOUNT` isn't modeled**, so the third trigger of the sequence family's Msg 11739 doesn't fire — real refuses a `NEXT VALUE FOR` in any statement under a set `ROWCOUNT`, alongside the `TOP` and `OFFSET` triggers that do ship.
  The option itself parses and is discarded ([`grammar.md`](grammar.md)), so enforcing the gate without the row limit would be the odder half to build alone.
