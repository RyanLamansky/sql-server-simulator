...
- **A zero-weight character compares equal to nothing** — `N'x' + NCHAR(0x00AD) = N'x'` (soft hyphen, and the ASCII control characters) is true here and false on real, so a row real excludes comes back (probed 2026-08-05 while pinning the LIKE matching model).
  `CompareInfo` gives those characters no weight where real gives them one; the companion divergence runs the other way and isn't a register entry — real expands a ligature (`N'ß' = N'ss'`) where `CompareInfo` doesn't.
  `LIKE` is unaffected by the first: it counts characters, so `LIKE N'x'` answers no there as real does → [`collations.md`](collations.md#known-gaps).
...
- **A qualified name that binds nowhere is Msg 207 rather than Msg 4104 outside the FROM-source check** — real splits by *what* failed: an unbindable **multi-part** identifier is Msg 4104 (`SELECT zz.id FROM t`), while a known qualifier's unknown column is Msg 207 (`SELECT t.nosuch FROM t`) — probed 2026-08-05.
  The simulator answers Msg 207 for both, so a **derived table** naming a sibling (`FROM t JOIN (SELECT t.id AS x) d ON 1 = 1`) reports 207 where real reports 4104, and so does a generator whose sibling reference is buried in a *scalar subquery* argument (`dbo.f((SELECT MAX(t.id)))`), which the FROM-source collector deliberately doesn't see into.
  The non-APPLY FROM-source case ships (see [`joins.md`](joins.md#a-non-apply-sources-arguments-cant-read-a-sibling)); generalizing it means classifying every unresolved name by whether its qualifier is in scope.
- **The `NEXT VALUE FOR` refusals past Msg 11719 / Msg 11720** — the nested-query family (Msg 11719) and the clause family (Msg 11720) ship (see [`sequences.md`](sequences.md#where-next-value-for-is-rejected)); three more are probed and unbuilt, each a flag at its own parse site.
  **Msg 11741** for an arm of `CASE` / `COALESCE` / `IIF` / `ISNULL` / `NULLIF` — real's message names `CHOOSE` too but *accepts* a `CHOOSE` argument, so the simulator should as well.
  **Msg 11725** for an aggregate's argument.
  **Msg 11721** for a statement carrying `DISTINCT` or a set operator — the `DISTINCT` half is a flag at the projection parse; the set-op half needs a per-branch counter, since the branch parses before its `UNION` is seen.
  All three still advance the sequence here, which is the state-leak half of the divergence.
- **`UPDATE <table> SET … FROM (<derived table>)` with the target named directly is Msg 208** — `UPDATE #u SET id = d.n FROM (SELECT 1 AS n) d` reports "Invalid object name '#u'" where real runs it (surfaced 2026-08-05 while probing the sequence contexts; the aliased spelling `UPDATE u … FROM #u u JOIN …` works).
  The target resolves only against the FROM's own sources, so a FROM that introduces no source for it loses it.
...
- **A parenthesized expression carrying a postfix `COLLATE` doesn't parse in a predicate** — `WHERE (a + b) COLLATE X LIKE 'y'` is Msg 4145 (`non-boolean type … near ')'`) here where real accepts it; the predicate parser reads the `(` as opening a parenthesized *condition* and then meets the `COLLATE` (probed 2026-08-05 while building the LIKE matrix, whose probes take a `CAST(… AS nvarchar(N)) COLLATE …` shape because of it).
  An unparenthesized operand (`a COLLATE X LIKE 'y'`) and the same parenthesized shape in a select list both parse.
