...
- **Where a module's missing object is reported** (probed 2026-09-26 against SQL Server 2025).
...
- **A table type's own constraints in the catalog** (probed 2026-09-26 against SQL Server 2025) — real lists a table type's PRIMARY KEY, UNIQUE, CHECK and DEFAULT as children of its `TT_…` object: in `sys.objects` / `sys.all_objects` (schema `sys`, `is_ms_shipped` 1, parent the type table), `sys.indexes` / `sys.stats` / `sys.index_columns` (an inline `INDEX` included, numbered ahead of the UNIQUE's index), `sys.default_constraints`, `sys.check_constraints` and `INFORMATION_SCHEMA.TABLE_CONSTRAINTS` (schema `sys`, table NULL) — but not `KEY_COLUMN_USAGE`.
  The simulator lists only the key constraints, in `sys.key_constraints`, because a table type holds its constraints as pending declarations its clones resolve; the catalog wants a shape of its own the way a multi-statement TVF's return table has one.
