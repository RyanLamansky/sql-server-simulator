...
- **`ORDER BY` inside a parenthesized INSERT source** — `INSERT INTO t (Id) (SELECT TOP 2 v FROM … ORDER BY v)` executes here where real raises **Msg 156** at the `ORDER` keyword, and does so even with the `TOP` that would license the clause in a derived table — a stricter rule than the Msg 1033 one the nested constructs take (probed 2026-08-06, alongside the parenthesized-source grammar itself → [`dml.md`](dml.md#insert--select)).
- **`DECLARE` and `PRINT` swallow a trailing token** — `DECLARE @x int = 1 zzz` and `PRINT 'x' zzz` execute here where real raises **Msg 102** at the stray token (probed 2026-08-06; `SET NOCOUNT ON zzz` is correctly rejected, because that parser leaves the cursor where the dispatch loop meets the token and tries to read a statement from it).
  The narrow per-statement shape the [trailing-token tightening](grammar.md#trailing-token-tightening) describes is what this wants; the general rule is documented there as too invasive.
