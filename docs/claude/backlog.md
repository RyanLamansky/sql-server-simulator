...
- **Double-quoted delimited identifiers** (`SELECT [x] AS "X"`, `"My Table"`) — the tokenizer has no `"` handling at all, so any double quote raises Msg 102 language-wide. Real SQL Server treats `"…"` as an identifier under the default `SET QUOTED_IDENTIFIER ON` (and as a string literal under OFF, which the SET accept-list parse-and-discards). Tokenizer-level feature touching every identifier site; surfaced by the SSMS shakedown (2026-07-15) while adding string-literal column aliases.
