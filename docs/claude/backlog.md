...
- **A SERIALIZABLE `MERGE` takes no writer fence** — a SERIALIZABLE `UPDATE` / `DELETE` fences its WHERE's interval in `RangeX-X` (see [`locking.md`](locking.md#what-a-serializable-writer-takes)), but `MERGE` still takes only table-IX plus row-X.
