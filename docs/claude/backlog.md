...
- **Spatial evaluation** — the value model and the **planar** measures ship (WKT read/write, SRID, Z/M, EMPTY, both binary encodings, the constructors, the structural members, and `geometry`'s `STArea` / `STLength` — see [`spatial.md`](spatial.md)). What remains:
  the **round-earth measures**, which need the *great elliptic arc* rather than a geodesic — probed 2026-07-31, Vincenty is 3.3 m off over Seattle→Paris while matching meridians to 0.07 mm, which identifies the curve; [`spatial.md`](spatial.md#the-great-elliptic-arc) carries the derivation and the reference values to test against;
