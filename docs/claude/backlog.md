...
- **`decimal` / `numeric` backed by .NET `decimal` — replace with a specialized SQL-decimal type** (user direction, 2026-08-05: worth taking on at some point; a lot of work).
  A purpose-built 38-digit type (four 32-bit limbs + sign + declared precision/scale, mirroring real's storage) eliminates the whole fidelity class the .NET backing causes rather than patching instances: the 28-significant-digit ceiling (declarations through `decimal(38, *)` parse but wide values aren't representable, surfacing as an unhandled `OverflowException` today), the trailing-zero capacity limit on the declared-scale stamp, the `DecimalMath` pre-scaling dance that keeps .NET's rounding out of digits real keeps, the `DEGREES`/`RADIANS` last-digit constant-precision divergence, and every future digit-38 arithmetic edge.
  Cost drivers to scope before starting: the arithmetic core (add/sub/mul/div/mod with real's per-operator scale rules and the probed truncate-vs-round split — the probe matrices from the division-truncation work are the acceptance tests), CAST/CONVERT + string parse/format, the aggregate accumulators (`SUM`/`AVG` at `decimal(38, s)`), the row encoder's byte layout (already 38-digit-shaped), the client boundary (`SqlValue` → .NET `decimal` for the reader **narrows** — probe what real's TDS does for >28-digit values vs what SqlClient can represent; `SqlDecimal` is the likely reader-surface answer, which is itself a public-surface question), and every arithmetic test's expected values re-anchored by probe.
  Suggested staging: the type + arithmetic core behind the existing seams first (`DecimalMath`, `PromoteForArithmetic` untouched in shape), then storage/casting, then the client boundary last since it's the only public-surface move.
