...
- **A SERIALIZABLE writer's fence past the single-table forms** — a SERIALIZABLE `UPDATE` / `DELETE` fences its WHERE's interval in `RangeX-X` (see [`locking.md`](locking.md#what-a-serializable-writer-takes)), but the joined forms and `MERGE` still take only table-IX plus row-X.
