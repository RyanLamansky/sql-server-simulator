...
- **Browse mode residue** — `KeyInfo` / `SET NO_BROWSETABLE` ship over the wire (hidden key and rowversion columns, TABNAME / COLINFO — see [`tds-endpoint.md`](tds-endpoint.md#browse-mode-commandbehaviorkeyinfo)); what's left is real's flattening of a derived table or view into its base tables for that metadata, the `FOR BROWSE` clause (Msg 156 here), and base-column metadata from the in-process reader's `GetSchemaTable`.
