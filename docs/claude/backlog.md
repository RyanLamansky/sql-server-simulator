...
- **DML through a row-limited or windowed view or CTE** — real writes through a `TOP` / `OFFSET` body or a window-function body to exactly the rows it yields, which is what makes the dedupe idiom `WITH d AS (SELECT *, ROW_NUMBER() OVER (PARTITION BY k ORDER BY v) rn FROM t) DELETE FROM d WHERE rn > 1` work (probed 2026-09-25); the simulator raises `NotSupportedException` for a non-positioned `UPDATE` / `DELETE` / `MERGE` through one.
  The write path selects base rows one at a time against the view's filter, so it needs the body run once with each output row's base address carried through the projection, window and row-limit stages; neither the heap enumeration's addresses nor the cursor machinery's row identities survive a window or `TOP` stage today.
  A multi-source CTE target is the same kind of gap: it doesn't reach the join-view path, which works from a stored view's text.
