...
- **Transient wrong results from aggregate / window queries under concurrency** — the `.vs/workload` streams intermittently produce result-signature mismatches vs live under 8 concurrent workers, always in templates that sort or window over an aggregate: WWI `stockitem.running_movement` (`SUM(Quantity) OVER (ORDER BY …)`, 1–5 of ~478 instances per 20k-op seeded run), AW `report.top_customers_in_territory` / `report.sales_by_territory` (`TOP … ORDER BY SUM(TotalDue) DESC` over a grouped derived table, 2–6 per run), AW `customer.running_total`. **Zero** divergences single-threaded (exhaustive all-key sweep of the WWI template matched live exactly), and any given instance matches on re-run — purely timing-sensitive. Reproduced on the pre-2026-07-10 commit via a clean worktree (0, 3 divergent across two seeded AW runs; 5 on WWI) and on the equality-prefix+range-continuation build (2, 6 on AW; 1 on WWI), so it long pre-dates that work and both builds flake at comparable rates. Read-only streams — no journal / version-store involvement — so suspicion falls on shared mutable state reached by concurrent executions of one cached plan through the aggregate/window buffer-sort paths (the plan cache's `VariableReference` co-fix covered variables; grouped/windowed decimal accumulation may hold per-execution state on the shared `Selection`). Transient wrong answers outrank most feature work — worth a dedicated hunt with a tight repro harness (N threads × one template, diff signatures per instance). Two harness quirks to fix while there: the divergence example line rebuilds parameters from the op seed and doesn't match the actual divergent instance, and divergent instances aren't re-run single-threaded to classify transient-vs-stable (`.vs/workload/Program.cs`).
