...
- Numeric parameters and return types and an alias type over numeric read `decimal`, as does a numeric column's name in a type-pair message (raised while binding, before references are marked); columns, variables, references and the catalog carry the name (see [`arithmetic.md`](arithmetic.md#numeric-vs-decimal-reported-type-name)).
