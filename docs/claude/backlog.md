...
- **`ORDER BY` inside a parenthesized INSERT source** — `INSERT INTO t (Id) (SELECT TOP 2 v FROM … ORDER BY v)` executes here where real raises **Msg 102** at the `ORDER` keyword, and does so even with the `TOP` that would license the clause in a derived table (probed 2026-08-06, alongside the parenthesized-source grammar itself → [`dml.md`](dml.md#insert--select)).
- **A reserved word is named without real's "the keyword" prefix** — `Incorrect syntax near 'BEGIN'.` here against real's `Incorrect syntax near the keyword 'BEGIN'.`, on the same Msg 102 (probed 2026-08-06).
  Wording only; the number, class and state match, and the simulator already echoes the source's own casing the way real does.
