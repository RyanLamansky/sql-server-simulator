...
- **Residual object-name swallow.**
  The remaining object-resolution sites that still resolve inline — DML target tables (INSERT / UPDATE / DELETE / MERGE), `NEXT VALUE FOR` sequences, XML schema collections — throw Msg 208 in skip mode, and an `ALTER TABLE … ADD` of columns to a missing table Msg 4902, which real defers the same way (its column errors wait behind the missing table, probed 2026-09-25); both are caught in `DispatchOneStatement`'s materialize-then-catch wrapper.
  `IsDeferrableNameResolutionError` is `{208, 4902}` (Msg 207 removed — it must reach the batch-aborting path per the matrix); the wrapper swallows the error, advances the cursor to the next statement boundary, and drops the statement with **no** `@@ERROR` / `InFlightError` mutation.
