...
- [x] **JSON_OBJECTAGG(k : v)** / **JSON_ARRAYAGG(expr)** — JSON aggregators (grouped + `OVER` window forms, incl. running/explicit frames). `JSON_ARRAYAGG` takes an in-parens `ORDER BY` (mutually exclusive with `OVER`); `JSON_OBJECTAGG` takes only the colon `key : value` form (no `key VALUE value`, no `ORDER BY`). Null-clause defaults match the scalar builders — `JSON_ARRAYAGG` → ABSENT ON NULL, `JSON_OBJECTAGG` → NULL ON NULL; empty input → NULL, all-absent group → `[]` / `{}`. Both return `nvarchar(max)`. Probing also surfaced and fixed a scalar-`JSON_OBJECT` default-null-clause bug (was ABSENT, should be NULL ON NULL). 🎯 closes **JSON (10/10)** with JSON_OBJECT / JSON_ARRAY / JSON_PATH_EXISTS from Tier 3. See [`json.md`](json.md).
...
- [x] **SESSION_CONTEXT(key)** + **sp_set_session_context** — per-session key/value store on `SimulatedDbConnection.SessionContext` (case-sensitive keys, `@read_only` → Msg 15664, NULL key → Msg 225, NULL key arg → Msg 8116). sql_variant value surfaces as nvarchar (proxy, like SERVERPROPERTY); the `int_col = SESSION_CONTEXT(...)` RLS pattern works via the comparison coercion path. See [`scalars.md`](scalars.md).
- [x] **CONTEXT_INFO()** + **SET CONTEXT_INFO** — 128-byte binary slot on `SimulatedDbConnection.ContextInfo` (right-padded / truncated to 128; NULL before first set). Literal-binary `SET` form only; a `@var` value side stays unmodeled (SET value parser accepts only a literal).
...
- [x] **OBJECT_DEFINITION(id)** — module source text for procs / views / DML + DDL triggers / scalar / inline / multi-statement functions, captured at CREATE / ALTER time on `SchemaObject.DefinitionText`. Byte-exact vs SQL Server (verbatim original CREATE text incl. comments / spacing; leading verb normalized to CREATE for ALTER / CREATE OR ALTER, the OR / ALTER keyword tokens removed but their whitespace preserved). NULL for non-modules / missing id / WITH ENCRYPTION. Shipped alongside `sys.sql_modules` (12 cols) + `INFORMATION_SCHEMA.ROUTINES.ROUTINE_DEFINITION` (nvarchar(4000), truncated). Also closed an adjacent `OBJECT_ID` gap — it now resolves DML triggers (`'TR'`), so `OBJECT_DEFINITION(OBJECT_ID('trg'))` works. See [`catalog-views.md`](catalog-views.md). Only documented divergence: leading whitespace / comment trivia *before* the CREATE keyword isn't captured (tokenizer skips it).
- [x] **CONNECTIONPROPERTY(name) / CURRENT_REQUEST_ID() / CURRENT_TRANSACTION_ID()** — session/request identity scalars. `CONNECTIONPROPERTY` → nvarchar proxy (`net_transport`='TCP', `protocol_type`='TSQL' probe-confirmed; unknown → NULL); `CURRENT_REQUEST_ID()` → 0; `CURRENT_TRANSACTION_ID()` → bigint approximated by the monotonic commit counter. **`SESSION_ID()` excluded — not a box-product function** (probe-confirmed Msg 195 on SQL Server 2025; it's a dedicated-SQL-pool/cloud surface, and `@@SPID` is the box session-id mechanism). Modeling it would diverge from the reference, same rationale as ANY_VALUE.
