...
- **`PWDENCRYPT(clear)`** — always emits the current SQL Server 2025 **`0x0300`** format: `0x0300 || salt(4) || PBKDF2-HMAC-SHA512(UTF-16LE(clear), salt, 100000 iterations, 64 bytes)` = **70 bytes** `varbinary` (probe-confirmed `DATALENGTH` 70). A fresh random salt per call, so successive hashes of the same password differ. NULL → NULL. (Undocumented sibling of `PWDCOMPARE`.)
- **`PWDCOMPARE(clear, hash [, version])`** — `int` `1`/`0` for match/mismatch. Recognizes both `0x0300` (PBKDF2, current) and legacy `0x0200` (single-pass `SHA-512(UTF-16LE(clear) || salt)`, SQL Server 2012–2022) by reading the version tag, so it verifies hashes from any of those engines. NULL `clear` or NULL `hash` → NULL; a short / malformed / unrecognized-version hash → 0 (probe-confirmed, no error). The optional third `version` argument (legacy upgrade hint) is accepted and ignored — real SQL Server ignores it for comparison too.

## `LOGINPROPERTY`

**`LOGINPROPERTY(login_name, property_name)`** (`Parser/Expressions/LoginProperty.cs`) — the simulator has no login model, so only the single fixed login (`dbo`, the placeholder `SUSER_NAME` reports) resolves; any other name behaves like a nonexistent login and returns **NULL** for every property (probe-confirmed: nonexistent login → NULL across the board). NULL login / NULL property / unrecognized property → NULL. Property names case-insensitive. For the fixed login, values are plausible constants matching the live probe's shape: `PasswordLastSetTime` → a fixed seed date `2020-01-01 00:00:00.000`; `BadPasswordTime` / `LockoutTime` → the `1900-01-01` "never" sentinel; `BadPasswordCount` / `HistoryLength` / `IsExpired` / `IsLocked` / `IsMustChange` → `0`; `DaysUntilExpiration` / `PasswordHash` / `PasswordHashAlgorithm` → NULL (a low-privilege login sees NULL for the hash on the live server too, and the simulator stores none); `DefaultDatabase` → the session's current database; `DefaultLanguage` → `us_english`. **Divergence**: real SQL Server projects each property as `sql_variant` with a per-property base type (`datetime` / `int` / `nvarchar` / `varbinary`); the simulator doesn't model `sql_variant`, so — following `SERVERPROPERTY` — every value surfaces as `nvarchar`, reached through implicit conversion when a caller casts.
