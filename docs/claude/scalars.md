...
An argument with **no declared width** contributes none, so there is no sum to take and the whole result falls back to the family container instead — the same fallback `LEFT` / `RIGHT` / `SUBSTRING` / `STUFF` / `REPLICATE` take for an operand they can't bound.
...
So do the `*FROMPARTS` parts (`int`), the `SWITCHOFFSET` / `TODATETIMEOFFSET` offset (`smallint`, a string passing) and `TEXTVALID`'s pointer (`varbinary`), and every one is judged while compiling, ahead of a `*FROMPARTS` scale's own refusal (probed 2026-09-26 against SQL Server 2025).
...
A bare `NULL` takes no part in CHOOSE's, LEAST's or GREATEST's type (`CHOOSE(2, NULL, 'x')` is `x`), DATEADD refuses one as its number with Msg 8116, UNICODE reads a binary as the UTF-16 an `nvarchar` would hold, and Msg 9810 names a datepart by its own name whatever abbreviation was written (`w` included, as weekday) — all probed 2026-09-25 against SQL Server 2025.
...
- **`@@VERSION`** — a multi-line banner mirroring the real SQL Server 2025 `@@VERSION` shape (`Microsoft SQL Server 2025 (RTM-CU7) (KB5096981) - 17.0.4065.4 (X64)` / build-date line / copyright / edition line), with the simulator's own identity (`Enterprise Developer Edition (64-bit) on SQL Server Simulator`) standing in for real's host-OS line.
