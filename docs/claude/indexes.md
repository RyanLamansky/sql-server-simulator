...
The ordered candidate list is still built eagerly (O(rows) addresses), so the win is "no sort + materialize only what's taken," not sublinear; a deep `OFFSET` streams past the skipped rows too, so its win is just the eliminated sort.

ORDER BY elimination is the one index optimization that's **observable if wrong** (it's the only place row order is guaranteed), so the bar to apply it is deliberately high — it declines (keeping the buffered sort) for: a nullable order column (its NULL-key rows aren't in the ordered view, which excludes any tuple with a NULL component), a **mixed-direction** multi-column order (the single ascending-by-value view serves only all-ASC forward or all-DESC reversed — per-column index direction isn't modeled), an order whose column sequence isn't a leading prefix of any key, an expression / ordinal sort key, `DISTINCT`, a SNAPSHOT / RCSI read (the version-chain sweep can't stay ordered), a tx-scoped row-lock plan, or a competing equality / IN / range seek on a leading key column the chosen prefix doesn't consume — including an IN-list on the first order column, where the composite equality seek (`a = @x AND b IN (…) ORDER BY b`) pins one column further than the ordered prefix and so wins. Keyset-style composite pagination (`WHERE a > @x OR (a = @x AND b > @y)`) isn't a single range, so it stays a buffered sort.
