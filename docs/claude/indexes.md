...
A `SELECT` from a single base table ordered by **one NOT-NULL leading key column** (ASC or DESC) skips the buffer-and-sort entirely: `TryApplyOrderedScan` (traced `OrderedScan(table)`) enumerates the column's ordered view (the same `SortedSet` the range seek builds, ascending or — for DESC — reversed) and routes to the streaming projector. The residual WHERE and projection preserve order, so the sort (and the full-result buffer) is gone, and `TOP` / a shallow `OFFSET` then materialize only the rows they need — `SELECT TOP 50 … ORDER BY id` over 100 000 rows dropped from ~65 ms/query (materialize + sort the whole table) to ~7 ms/query (stream, materializing only the 50 taken) — ~8.7×. The ordered candidate list is still built eagerly (O(rows) addresses), so the win is "no sort + materialize only what's taken," not sublinear; a deep `OFFSET` streams past the skipped rows too, so its win is just the eliminated sort. A same-column range bound folds into the ordered scan (`WHERE id > 100 ORDER BY id` stays narrowed *and* ordered).

ORDER BY elimination is the one index optimization that's **observable if wrong** (it's the only place row order is guaranteed), so the bar to apply it is deliberately high — it declines (keeping the buffered sort) for: a nullable order column (its NULL-key rows aren't in the ordered view, which excludes NULLs), multiple ORDER BY items, an expression / ordinal sort key, `DISTINCT`, a SNAPSHOT / RCSI read (the version-chain sweep can't stay ordered), a tx-scoped row-lock plan, or a competing equality / other-leading-column seek (the narrower seek plus a small sort wins). Multi-column ORDER BY elimination (composite key) is not modeled — the ordered view is single-component.
