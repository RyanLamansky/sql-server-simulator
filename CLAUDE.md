...
- **DML + DDL triggers** (`CREATE TRIGGER` incl. `ON DATABASE`, `INSERTED`/`DELETED`, `TRIGGER_NESTLEVEL`, `sys.triggers`), the **trigger atomic scope** (the body has none of its own — the firing statement and everything its triggers wrote roll back as one unit, including writes by modules the body called) and **Msg 3616** (a severity-≥11 error the body's own TRY/CATCH swallowed still aborts), the **`UPDATE(col)` / `COLUMNS_UPDATED()`** change-detection intrinsics, and **trigger-body result sets** (a body `SELECT` is the firing statement's result set; the body's rows-affected stays suppressed) → [`triggers.md`](docs/claude/triggers.md).
