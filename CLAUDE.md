...
- **`GRANT` / `REVOKE` / `DENY`** — securable resolution and covering/scope-walk enforcement across the DML, EXECUTE and DDL gates, role closure, DENY-beats-GRANT, ownership chaining, module `WITH EXECUTE AS`, and grant-option delegation; column-level grants on tables and views (Msg 230 naming the view's *own* column); **reference provenance** — a name written through a synonym is checked against the synonym, never the base object behind it; principal and login DDL, server roles and server-scope grants, and the principal scalars.
  ...
  **Cross-database references resolve the login's user in the *target*** — its grants answer and a denial names that database (**Msg 229**), a login with no user there is **Msg 916**, a database-scoped identity (`EXECUTE AS USER`, a module's named-user frame, an application role) can't cross at all, an ownership chain breaks at the database boundary, and `USE` asks the same question and rebinds the session's user on success.
...
- **Per-column / per-expression collation, coercibility precedence, Msg 468/457/451 cross-collation enforcement (bound at **compile time**, so an empty rowset and a module body at CREATE raise what real raises — with Msg 451 naming the clause and 1-based ordinal of the output slot `CONCAT` / `CONCAT_WS` couldn't settle, and staying silent where an assignment target supplies the collation), recognized catalog, `#temp` inheritance, name regimes (fullwidth/decomposed fold, hash canonicalization, variable names), and the per-collation ANSI storage code page** (`varchar`/`char` stored in the collation's own page — SBCS, DBCS and UTF-8 — the byte-counted `varchar(N)` budget, and Msg 459 for the Unicode-only collations) → [`collations.md`](docs/claude/collations.md).
...
- **Cross-server DML** (`INSERT`/`UPDATE`/`DELETE`/`MERGE` through a 4-part linked-server name) → `NotSupportedException` via `BatchContext.RejectCrossServerMutation`; open a connection on the target `Simulation` instead.
  ...
  Cross-*database* DML inside one `Simulation` ships, permission resolution and snapshot stamps included; the one per-database surface still reading the session's database is the catalog-view metadata filter.
...
- **`ALTER DATABASE … SET` / `COLLATE`** — see [`database-options.md`](docs/claude/database-options.md).
  Most options parse-and-discard; `COMPATIBILITY_LEVEL`, `ALLOW_SNAPSHOT_ISOLATION`, `READ_COMMITTED_SNAPSHOT`, `RECURSIVE_TRIGGERS` are load-bearing, and every option lands on the **named** database (`CURRENT` for the session's; an unhosted name is Msg 5011).
...
- **Programmable-object top-level gaps**: CLR procedures / TVFs / aggregates / UDTs (CLR *scalar functions* ship — see [`clr-assemblies.md`](docs/claude/clr-assemblies.md)), logon triggers, INSTEAD OF UPDATE/DELETE on non-updatable views, JOIN-view single-base UPDATE/DELETE, OUTPUT through views, multi-source alias-form UPDATE/DELETE through views (Msg 4405).
  ...
  A module body binds at CREATE and its shape is checked there, but `NEWSEQUENTIALID()` in a function body reports the simulator's Msg 302 (real's outside-a-`DEFAULT` gate) where real reports Msg 443, and a bind abandoned at a deferral leaves the last-statement rule unrun.
