...
- **`Database`** (internal) = one database.
  ...
  Database ids are a fixed map — master = 1, tempdb = 2, model = 3, msdb = 4; user databases from 5 in name order — single source of truth is `Simulation.SystemDatabaseIds` + `DbId.DatabasesWithIds`, consumed by `DB_ID`/`DB_NAME`, `sys.databases`, `OBJECT_NAME`, and `DBCC SHRINKDATABASE`.
...
- **`Cast` / coercion error paths** (CAST/CONVERT narrow targets, TRY_CAST/TRY_CONVERT swallow set, PARSE/TRY_PARSE culture-aware parsing, ANSI type-name synonyms `integer`/`dec`/`character`/`double precision`/`character varying`/`national character [varying]`/`binary varying`/`national text`) → [`casting.md`](docs/claude/casting.md).
...
- **`Selection`, aggregates, window functions, set ops, CASE, OFFSET/FETCH, `TOP n [PERCENT] [WITH TIES]`, `WINDOW w AS (…)` named-window clause (bare `OVER w`; `OVER (w …)` inheritance deferred), `TABLESAMPLE` (parsed, approximated as all-rows)** → [`query.md`](docs/claude/query.md).
...
- **New top-level statement parser or dispatch-loop separator rules, double-quoted identifiers / QUOTED_IDENTIFIER, trailing-token tightening (`SELECT … LIMIT n` → Msg 102), `ALTER TABLE … ADD COLUMN` rejection (Msg 156)** → [`grammar.md`](docs/claude/grammar.md) + [`control-flow.md`](docs/claude/control-flow.md).
