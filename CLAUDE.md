...
- **Built-in scalars** — the math / date / string / bit families, HASHBYTES / CHECKSUM, FORMAT / FORMATMESSAGE, COMPRESS / DECOMPRESS, the built-in TVFs (`STRING_SPLIT` / `GENERATE_SERIES`), `@@`-constants and statistical counters, the session-state store (SESSION_CONTEXT / CONTEXT_INFO / connection scalars), the `sys.fn_*` system functions, legacy text-pointer and placeholder scalars, ODBC escape sequences (`{d …}` literals, `{fn …}` calls), and the **shared integer-argument narrowing seam** — an id / position / count / offset outside its parameter's declared range reports SQL Server's own conversion error keyed by source type (Msg 8115 / 220 / 232 / 237 / 248), a system procedure's parameter Msg 8114, a cursor FETCH offset literal Msg 1080, and the bit family's own Msg 8116 / 9838 / 9839 argument rules → [`scalars.md`](docs/claude/scalars.md).
...
- **Cursors** — the full DECLARE/OPEN/FETCH/CLOSE/DEALLOCATE lifecycle, STATIC / KEYSET / DYNAMIC sensitivity and the scrollability each implies (with the Msg 16911 / 16925 split between a forward-only cursor and a dynamic one refusing ABSOLUTE, and a cursor naming no sensitivity staying forward-only whatever it converted to), GLOBAL / LOCAL scope, cursor variables, the concurrency modes (SCROLL_LOCKS / OPTIMISTIC) and their status scalars, and **multi-source and deferred-source cursors** — a JOIN / comma-FROM / self-join cursor, and one reading through a **derived table, CTE, APPLY right side or view** (to any nesting depth), is DYNAMIC (or KEYSET) like real's, re-folding the live base heaps per FETCH over the flattened tuple of row addresses the whole shape reads, with `WHERE CURRENT OF` addressing the reference **as written** — a view is named by the view (its CHECK OPTION then applies, Msg 550), a query body is transparent and named by its base table, and everything else is Msg 16933 (plus Msg 16947 for a NULL-extended outer-join / OUTER APPLY side or a keyset hole, Msg 16931 for a cursor merely unpositioned, and Msg 16961 for a self-join's first-instance binding).
  **Row-limited cursors** (`TOP n` / `PERCENT` / `WITH TIES` / `OFFSET…FETCH`, on the statement or inside a body the cursor follows) convert to **KEYSET** like real's, the limit choosing membership at OPEN while later FETCHes re-read the frozen key set — except a limit inside a *view* body, which real re-evaluates per FETCH so a member the view no longer returns reports `@@FETCH_STATUS = -2`; a **`FOR SYSTEM_TIME`** cursor is the read-only snapshot real reports, in all five forms.
  An **ORDER BY no index delivers** converts the same way (a sorted result is a materialized one), off a leading-prefix rule that resolves each item down to its base column through ordinals, aliases and view / derived-table bodies, matches it against the table's keys in a consistent direction, and carries a nonclustered index's clustering key after its own; and **`TYPE_WARNING`**'s Msg 16956 reports every such conversion, against the sensitivity the keywords *imply* as much as one spelled out → [`cursors.md`](docs/claude/cursors.md).
- **CTE shapes / recursive-CTE error handling**, including the recursive member's construct restrictions — DISTINCT (Msg 460), TOP / OFFSET (461), outer join (462), GROUP BY / HAVING / aggregate (467), each covering the member's whole text including nested subqueries — and **where a `WITH` prefix may appear**: on a statement, and on a stored body (view, inline TVF, cursor declaration) through the shared body-parse seam, but never on a parenthesized query (derived table / subquery / scalar UDF `RETURN (…)` / `MERGE … USING (…)` → Msg 156, matching real) → [`ctes.md`](docs/claude/ctes.md).
...
- **Locking, MVCC, SNAPSHOT/RCSI, deadlock/timeout, lock-related DMVs** — including the **instance-wide commit-Xid sequence** (real's server-scoped transaction sequence number): a SNAPSHOT transaction fixes one stamp at its first data-access statement and reads *every* database as of that instant, while RCSI re-stamps per statement, and both the versioning flags and the Msg 3952 gate follow the table's own database rather than the session's.
  Plus **key-range locks** — a SERIALIZABLE / `HOLDLOCK` reader fences the *value interval* its predicate pins on an indexed leading column (`=` / `>` / `<` / `BETWEEN` / `IN`) rather than real's key-anchored gaps, so phantom protection is exact while two transactions over disjoint ranges stop blocking each other; the four `RangeS-S` / `RangeS-U` / `RangeX-X` / `RangeI-N` modes with their own compatibility family, a writer-side `RangeI-N` probe that runs **whatever the writer's own isolation level** (including the post-update image, which is the only thing that reveals a row moving *into* a fence), the `KEY` / `RangeS-S` `sys.dm_tran_locks` projection, and the whole-table S every other read shape still falls back to → [`locking.md`](docs/claude/locking.md).
...
- **`xml` type, XML schema collections, XML methods** (`.value()` / `.nodes()` / `.query()` / `.exist()` via an XQuery-subset evaluator — location steps with **value, existence and positional predicates**, the general (`=` `!=` `<` …) and value (`eq` `ne` `lt` …) comparison operators with XQuery's untyped-atomic coercion rule (`[@x=1]` compares numerically, `[@x="1"]` by code point) and the existential semantics that make `!=` not the complement of `not(=)`, `and` / `or` / `not()`, arithmetic, and the built-in function library — carrying the diagnostics real settles **statically**, so they fire while the SQL statement parses and over an empty rowset: Msg **2203** (a predicate that is neither numeric, boolean nor a node sequence), **2234** (incompatible operand types), **2389** (a value comparison, a singleton-parameter function or `.value()` itself over a path real types as a sequence — only positional narrowing at *every* step satisfies it, so `(/r/a)[1]` reads where `/r/a[1]` raises), **2395** (an unknown function), **2229** (an undeclared prefix), **2209** and **9335** — plus **`.modify()` XML-DML** — `insert` / `delete` / `replace value of` over the same paths, legal only as `SET @x.modify(…)` or an UPDATE's `SET col.modify(…)` where it desugars to an ordinary column assignment so OUTPUT / triggers / `@@ROWCOUNT` follow, with real's compile-time static target typing behind Msg 2226 / 2240 / 2249 / 2337 / 2356 / 2264 / 2207 / 2258, the position errors Msg 8137 / 8113 / 258 / 5302, and the normalized re-serialization an edited instance comes back in), XML indexes (with real's dedicated per-table **256000+ `index_id`** range, which `sys.index_columns` and the internal node table's name follow), and the `FOR XML` result-serialization clause — including **EXPLICIT mode's universal table** (the `Tag` / `Parent` protocol that builds the tree from consecutive rows in the order they arrive, the `ElementName!TagNumber!AttributeName[!Directive]` column convention with the whole directive set — `element` / `elementxsinil` / `xml` / `xmltext` / `cdata` / `hide` / the identity family — and Msg 6801 / 6802 / 6803 / 6804 / 6805 / 6806 / 6807 / 6812 / 6813 / 6815 / 6817 / 6820 / 6824 / 6825 / 6826 / 6827 / 6833 / 6834 / 6835 / 6859 / 3625), the **once-each option rule** (a repeated `TYPE` / `ROOT` / `ELEMENTS` / `BINARY BASE64` is Msg 102 against the clause's own `XML` keyword), the **XML-name split** (RAW / AUTO escape a name XML can't carry as `_xHHHH_`, off the XML 1.0 fourth-edition `Name` production real classifies by, while PATH's aliases and the explicit `RAW('elem')` / `PATH('row')` / `ROOT('name')` arguments reject it — EXPLICIT does neither and writes them verbatim: Msg 6850 / 6846 / 6867 / 6849), the **`WITH XMLNAMESPACES`** prefix (which element each mode declares the bindings on, the prefixed names it legalizes, and Msg 6868–6874), the **`BINARY BASE64`** option and the legacy `dbobject/TABLE[@PK='V']/@COLUMN` reference AUTO writes without it (Msg 6829 / 6830 / 6831), **AUTO over a set-operation result** (flattened to one level named after the first branch's first source), and the **Msg 6819** refusal of the clause on the SELECT an `INSERT … SELECT` / `SELECT … INTO` / `SELECT @v = …` consumes (FOR JSON's own Msg 13602, except the assignment form where real reports FOR XML's error for both).
...
- **`geography` / `geometry` types** — the parsed value model (WKT read/write with real's 24xxx validation failures, canonical rendering, per-value SRID, Z / M, EMPTY), the UDT serialization it stores and the OGC / ISO well-known binary `STAsBinary` produces, the `Parse` / `ST<Kind>FromText` / `ST<Kind>FromWKB` / `Point` constructor family, and the structural member surface with real's property-vs-method form split (Msg 6592 / 6506 / 6595) — plus spatial indexes and the catalog views.
  ...
  `geometry`'s **topological predicates** evaluate over a hand-rolled DE-9IM engine — the eight named predicates as masks over `STRelate`'s matrix (with real's non-symmetric `STCrosses` dimension gate and its empty/empty `STEquals` exception), the mod-2 line boundary and per-class collection unions, `STIsValid`'s OGC rules including ring-touch interior connectivity, and the **Msg 24144** gate an invalid instance puts on most of the instance surface (`STAsText` / the ordinate reads / `STLength` / `STIsRing` / `STSrid` / `STIsValid` tolerate it; everything structural or topological raises).
  Orientation runs in `double` under a roundoff filter, which is what real's own near-collinearity answers show.
  `geography`'s topological predicates and the constructive operations parse but don't evaluate yet → [`spatial.md`](docs/claude/spatial.md).
- **`ALTER DATABASE SET <option>` accept-list + database-level `COLLATE` clause**, including the two cross-database widening flags (`TRUSTWORTHY` / `DB_CHAINING`, their `sys.databases` projection, real's shipped system-database defaults, and the Msg 15309 / 5600 refusals on the databases real pins) → [`database-options.md`](docs/claude/database-options.md).
...
- **Key-range coverage past a single-column sargable predicate** — a SERIALIZABLE / HOLDLOCK read fences a key range when its predicate bounds the **leading** column of some key or index; a whole-table scan, an unindexed or composite-key predicate, an `ORDER BY`-eliminated ordered scan and a view / multi-source read all keep the whole-table S, which is what real degenerates to for the unindexed cases.
  `RangeS-U` / `RangeX-X` are defined and in the matrix but never acquired (`UPDLOCK` / `XLOCK` are read ahead of the isolation level, so they keep their row-U / row-X plan) — see [`locking.md`](docs/claude/locking.md#key-range-locks).
...
- **`ALTER DATABASE … SET` / `COLLATE`** — see [`database-options.md`](docs/claude/database-options.md).
  Most options parse-and-discard; `COMPATIBILITY_LEVEL`, `ALLOW_SNAPSHOT_ISOLATION`, `READ_COMMITTED_SNAPSHOT`, `RECURSIVE_TRIGGERS`, `TRUSTWORTHY`, `DB_CHAINING` are load-bearing, and every option lands on the **named** database (`CURRENT` for the session's; an unhosted name is Msg 5011).
- Legacy `READTEXT` / `WRITETEXT` / `UPDATETEXT`; binary `SUBSTRING` (real slices a `varbinary` / `image`, the simulator raises Msg 8116) — see [`scalars.md`](docs/claude/scalars.md#legacy-lob-arguments).
...
- **Programmable-object top-level gaps**: CLR procedures / TVFs / aggregates / UDTs (CLR *scalar functions* ship — see [`clr-assemblies.md`](docs/claude/clr-assemblies.md)), logon triggers, INSTEAD OF UPDATE/DELETE on non-updatable views, single-base INSERT through a JOIN view and any DML through a view *over* a JOIN view (the single-base **UPDATE** ships — see [`programmable.md`](docs/claude/programmable.md#update-through-a-join-view)), OUTPUT through views, multi-source alias-form UPDATE/DELETE through views (Msg 4405).
