...
- **`hierarchyid` byte-identical CAST encoding** — currently simulator-native; cross-engine byte transfer deferred. (`geography` / `geometry` byte-identical CAST + TDS UDT wire form **now ship** for the modeled 2D shapes — DacFx bacpac export of WWI's spatial columns works.) See [`hierarchyid.md`](docs/claude/hierarchyid.md), [`spatial.md`](docs/claude/spatial.md).
