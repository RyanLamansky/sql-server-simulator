...
- **Cursors** — the full DECLARE/OPEN/FETCH/CLOSE/DEALLOCATE lifecycle, STATIC / KEYSET / DYNAMIC sensitivity and the scrollability each implies (with the Msg 16911 / 16925 split between a forward-only cursor and a dynamic one refusing ABSOLUTE), GLOBAL / LOCAL scope, cursor variables, the concurrency modes (SCROLL_LOCKS / OPTIMISTIC) and their status scalars, and **multi-source cursors** — a JOIN / comma-FROM / self-join cursor is DYNAMIC (or KEYSET) like real's, re-folding the live base heaps per FETCH over a tuple of per-source row addresses, with `WHERE CURRENT OF` reaching whichever participating table the statement names (Msg 16933 for one the cursor doesn't read, Msg 16947 for a NULL-extended outer-join side, Msg 16961 for a self-join's first-instance binding) → [`cursors.md`](docs/claude/cursors.md).
...
- **Name resolution, schema lookup, CREATE / DROP / ALTER SCHEMA TRANSFER, CREATE / DROP DATABASE (stored smallest-free db_ids with freed-id reuse), the `OBJECT_*` / `SCHEMA_*` / `DB_*` scalars, cross-database reads *and writes* (three-part DML / DDL, with the rowversion counter, version store and trigger dispatch following the target database)**, and **synonyms** — `CREATE` / `DROP SYNONYM`, redirection at every reference site (FROM / DML / EXEC / function call), the `sys.synonyms` + `sys.objects` projection and `OBJECT_ID` / `OBJECTPROPERTYEX` reads, deferred base resolution (Msg 5313 / 470 at first use), and the Msg 2714 / 3705 collision rules in both directions → [`schemas.md`](docs/claude/schemas.md).
...
- **Cross-server DML** (`INSERT`/`UPDATE`/`DELETE`/`MERGE` through a 4-part linked-server name) → `NotSupportedException` via `BatchContext.RejectCrossServerMutation`; open a connection on the target `Simulation` instead.
  ...
  Cross-*database* DML inside one `Simulation` ships, minus two per-database surfaces that still read the session's database — the permission check and a SNAPSHOT / RCSI reader's snapshot stamp.
...
- **`ALTER DATABASE … SET` / `COLLATE`** — see [`database-options.md`](docs/claude/database-options.md).
  Most options parse-and-discard; `COMPATIBILITY_LEVEL`, `ALLOW_SNAPSHOT_ISOLATION`, `READ_COMMITTED_SNAPSHOT`, `RECURSIVE_TRIGGERS` are load-bearing.
...
- **Programmable-object top-level gaps**: CLR procedures / TVFs / aggregates / UDTs (CLR *scalar functions* ship — see [`clr-assemblies.md`](docs/claude/clr-assemblies.md)), logon triggers, INSTEAD OF UPDATE/DELETE on non-updatable views, JOIN-view single-base UPDATE/DELETE, OUTPUT through views, multi-source alias-form UPDATE/DELETE through views (Msg 4405).
  ...
  A module body binds at CREATE, but the **body-shape** rules it can't reach by parsing don't: Msg 455 (a function's last statement must be `RETURN`), Msg 444 (a body `SELECT` returning to the client) and Msg 443 (a side-effecting operator inside a function) go unraised, and a column that resolves per row (`WHERE` / `HAVING` / `MERGE`'s `ON` / a `SET`'s value) isn't reached by a bind that walks no rows.
