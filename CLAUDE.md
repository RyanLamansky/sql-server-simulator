...
- `RAISERROR`, stored procs / UDFs. `BEGIN ATOMIC` / `BEGIN DISTRIBUTED TRANSACTION` raise `NotSupportedException` at dispatch. Value-form `RETURN N` raises Msg 178 (reserved for the stored-proc / function scope, neither modeled yet). TRY/CATCH + THROW + live `@@ERROR` + `ERROR_*()` functions ship — see the TRY/CATCH section.
