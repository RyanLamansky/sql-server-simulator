...
- **`Selection`, aggregates, window functions, set ops, CASE, OFFSET/FETCH, `TOP`, named windows, `TABLESAMPLE`, the derived-table column-alias list (`(SELECT …) s(a, b)`, with Msg 8155 / 8156 / 8158 / 8159)** — including the aggregate/GROUP BY binding rules (Msg 130 / 8117 / 144 / 164, via `ParserContext` parse-time counters rather than a tree walk) and the grouped-query window shapes (a window spans the whole grouped result, `ROLLUP` / `CUBE` / `GROUPING SETS` subtotal rows included) → [`query.md`](docs/claude/query.md).
