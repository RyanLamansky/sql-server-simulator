...
internal sealed partial class Selection
{
    ...
    /// functions (currently <c>ROW_NUMBER() OVER(...)</c>). Buffers all
    /// post-WHERE tuples, partitions by each window's PARTITION BY keys,
    /// sorts each partition by the window's ORDER BY keys, assigns row
    /// numbers per partition, then walks the buffer in original order
    /// binding each window expression's per-tuple value before running
    /// the projection. Falls through to the same OFFSET / TAKE / DISTINCT
    /// / ORDER BY post-processing as <see cref="ProjectBuffered"/>.
    /// </summary>
