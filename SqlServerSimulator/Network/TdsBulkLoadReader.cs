...
/// writes for result rows, terminated by a DONE token. Each decoded value is a
/// <see cref="SqlValue"/> typed at its wire type; the engine coerces it to the
/// destination column on insert. This is the read mirror of the result-set
/// value codec — the value formats are identical, only the direction differs.
/// </summary>
internal static class TdsBulkLoadReader
{
    ...
    public static List<SqlValue[]> ReadRows(byte[] payload)
    {
        ...
        var columns = new BulkColumn[columnCount];
        for (var i = 0; i < columnCount; i++)
        {
            _ = reader.ReadUInt32(); // UserType
            _ = reader.ReadUInt16(); // Flags
            columns[i] = ReadColumnMetadata(reader);
    ...
    private static BulkColumn ReadColumnMetadata(Reader reader)
    {
        var token = reader.ReadByte();
#pragma warning disable SSS005 // Grouped by TDS TYPE_INFO shape (fixed-length / decimal / temporal / string / binary), not numeric order.
        switch (token)
        {
            ...
            case 0x7F: // INT8TYPE
                return new BulkColumn(token, 0, 0, 0, false);
            case 0x26: // INTN
            case 0x68: // BITN
            case 0x6D: // FLTN
            case 0x6E: // MONEYN
            case 0x6F: // DATETIMN
            case 0x24: // GUIDN
                return new BulkColumn(token, reader.ReadByte(), 0, 0, false);
            case 0x6A: // DECIMALN
            case 0x6C: // NUMERICN
                {
                    ...
                    return new BulkColumn(token, declared, precision, scale, false);
                }
            ...
            case 0x2B: // DATETIMEOFFSETN
                return new BulkColumn(token, 0, 0, reader.ReadByte(), false);
            case 0xA7: // BIGVARCHAR
            case 0xAF: // BIGCHAR
                {
                    var maxLength = reader.ReadUInt16();
                    var utf8 = ReadCollation(reader);
                    return new BulkColumn(token, maxLength, 0, 0, utf8);
                }
            ...
            case 0xEF: // NCHAR
                {
                    ...
                    return new BulkColumn(token, maxLength, 0, 0, false);
                }

            case 0xA5: // BIGVARBINARY
            case 0xAD: // BIGBINARY
                return new BulkColumn(token, reader.ReadUInt16(), 0, 0, false);
            case 0xF1: // XML
                {
            ...
            default:
                throw new NotSupportedException($"The network listener does not decode bulk-load column type token 0x{token:X2}.");
        }
#pragma warning restore SSS005
    }

    private static NotSupportedException Unsupported(string feature) =>
        new($"The network listener does not accept {feature} columns in a bulk-load (SqlBulkCopy) stream.");
    ...
    /// <summary>One bulk-load column's TYPE_INFO and its per-value decoder.</summary>
    private sealed class BulkColumn(byte token, int declaredOrMax, byte precision, byte scale, bool utf8)
    {
        public SqlValue ReadValue(Reader reader) => token switch
        {
            ...
            0xF1 => ReadXml(reader),
            _ => throw new InvalidDataException($"No bulk-load value decoder for type token 0x{token:X2}."),
        };

        private SqlValue ReadIntN(Reader reader)
        {
        ...
        private SqlValue ReadFloatN(Reader reader)
        {
            var length = reader.ReadByte();
            if (length == 0)
        ...
        private SqlValue ReadMoneyN(Reader reader)
        {
            var length = reader.ReadByte();
            if (length == 0)
        ...
        private SqlValue ReadDateTimeN(Reader reader)
        {
            var length = reader.ReadByte();
            if (length == 0)
        ...
        private SqlValue ReadDecimal(Reader reader)
        {
            // SqlClient's SqlBulkCopy writes every non-NULL numeric value as a
            // sign byte followed by a fixed 16-byte little-endian mantissa (17
            // value bytes), regardless of the precision-implied width the
            // length byte reports (probe-confirmed 2026-07-18: decimal(5,2) and
            ...
            {
                if (magnitude[i] != 0)
                    throw new NotSupportedException("A decimal bulk-load value exceeds the range of System.Decimal.");
            }
        ...
        private SqlValue ReadTime(Reader reader)
        {
            var length = reader.ReadByte();
        ...
        private SqlValue ReadDateTime2(Reader reader)
        {
            var length = reader.ReadByte();
        ...
        private SqlValue ReadDateTimeOffset(Reader reader)
        {
            var length = reader.ReadByte();
        ...
        private SqlValue ReadAnsiString(Reader reader)
        {
        ...
        private SqlValue ReadBinary(Reader reader)
        {
            if (declaredOrMax == 0xFFFF)
            {
        ...
        private static SqlValue ReadXml(Reader reader)
        {
        ...
        private static SqlValue? ByteLenNull(Reader reader, SqlType nullType) =>
            reader.ReadByte() == 0 ? SqlValue.Null(nullType) : null;

        private static SqlValue ReadFixedMoney(Reader reader)
        {
        ...
        private static SqlValue ReadFixedSmallDateTime(Reader reader)
        {
        ...
        private static SqlValue ReadFixedDateTime(Reader reader)
        {
            var payload = reader.ReadBytes(8);
    ...
    /// <summary>A forward-only, bounds-checked cursor over a bulk-load payload.</summary>
    private sealed class Reader(byte[] data)
    {
        ...
        private static InvalidDataException Truncated() =>
            new("The bulk-load stream ends before a value was fully read.");
    }
}
