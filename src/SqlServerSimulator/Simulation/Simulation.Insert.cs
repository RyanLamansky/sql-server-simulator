...
partial class Simulation
{
    ...
    private static SimulatedStatementOutcome ProcessHeapInsert(HeapTable destinationTable, ParserContext context, Selection.DmlTopLimit? top, MultiPartName destinationName, View? destinationView = null, JoinViewInsertPlan? joinViewPlan = null)
    {
        ...
        // a SELECT source), an implicit one against the table definition
        ...
        if (hasExplicitColumnList)
        {
            ...
            while (true)
            {
                ...
                // A computed column is refused for merely being named, whatever
                // value the tuple pairs with it (Msg 271 even for DEFAULT).
                // A rowversion / GENERATED ALWAYS column is not: naming it is
                // legal as long as its cell is the DEFAULT keyword, which is
                // the escape hatch Msg 273 / 13536 themselves advertise. That
                // check is per cell, so it waits for the tuples —
                // RejectExplicitAutoGeneratedValues below.
                if (tableColumn.Computed is not null)
                    throw SimulatedSqlException.ColumnCannotBeModified(tableColumn.Name);
                if (usedColumns.Contains(tableColumn))
                    throw SimulatedSqlException.ColumnAssignedMoreThanOnce(tableColumn.Name);
        ...
        {
            // No column list: the positional list is every column but the
            // identity and the computed ones. Identity drops out whatever
            // IDENTITY_INSERT says — probe-confirmed, which is why the ON
            // case can only ever end in Msg 8101 (a value too many) or
            // Msg 545 (the identity left unsupplied) and never succeeds
            // without a column list. Rowversion and GENERATED ALWAYS columns
            // stay *in*: they occupy a position that has to be filled with
            // the DEFAULT keyword, so `INSERT INTO t VALUES (1, 2)` against
            // `t(a, ts rowversion, b)` is Msg 213, not a two-column insert.
            // When the target is a view, the implicit list is the view's
            // projected columns mapped to their base ordinals; base columns
        ...
        if (context.Token is ReservedKeyword { Keyword: Keyword.Values })
        {
            ...
            RejectValuesArityMismatch(valueTuples, destinationColumns, hasExplicitColumnList, identityColumn, destinationTable);
        }
        ...
        // source is settled so the arity diagnostics — including the SELECT
        // source's own, raised inside ExecuteSelectSource — report first, and
        // ahead of the identity gates below, which is the order real reports a
        // statement offending both (probe-confirmed: naming an identity and a
        // rowversion column together reports Msg 273, not Msg 544).
        RejectExplicitAutoGeneratedValues(destinationTable, destinationColumns, valueTuples, context);
    ...
    /// <summary>
    /// An <c>INSERT … VALUES</c> list is capped at 1000 rows (Msg 10738, while
    /// compiling); a <c>VALUES</c> table constructor in FROM, including one an
    /// <c>INSERT … SELECT</c> reads, is not (probe-confirmed against SQL
    /// Server 2025).
    /// </summary>
    private static void RejectTooManyValueRows(List<Expression[]> tuples)
    {
        if (tuples.Count > 1000)
            throw SimulatedSqlException.TooManyInsertRowValues();
    }

    /// <summary>
    /// Parses <c>VALUES</c> tuples, measures them against
    /// <paramref name="destinationColumns"/> (Msg 109 / 110 / 213, then
    /// Msg 10709 for ragged tuples) and evaluates them. Used by the
    /// INSTEAD OF INSERT path, whose destination is a view projection with
    /// no identity column of its own to attribute a surplus value to.
    /// </summary>
    private static List<SqlValue[]> EvaluateArityCheckedTuples(ParserContext context, HeapColumn[] destinationColumns, bool hasExplicitColumnList)
    {
        ...
        return EvaluateParsedTuples(tuples, context.Batch, out _);
    }
    ...
    /// <c>CREATE PROCEDURE</c> whose body carries it (probe-confirmed) — which
    /// is why the check reads the parsed tuples rather than the evaluated rows
    /// and runs regardless of <c>IsSkipping</c>.
    /// </summary>
    /// <remarks>
    /// Which error depends on whether the statement wrote its own column list:
    /// <list type="bullet">
    /// <item>explicit list — <strong>Msg 110</strong> when the tuple is wider
    /// than the list, <strong>Msg 109</strong> when it is narrower;</item>
    /// <item>implicit list — <strong>Msg 213</strong>, except that a surplus
    /// value against a table carrying an identity column reports
    /// <strong>Msg 8101</strong> instead, real reading the extra value as an
    /// attempt to supply the identity (probe-confirmed for any surplus, not
    /// merely one).</item>
    /// </list>
    ...
    private static void RejectValuesArityMismatch(
        ...
        HeapColumn? identityColumn,
        HeapTable? destinationTable)
    {
        var arity = valueTuples[0].Length;
        for (var i = 1; i < valueTuples.Count; i++)
        {
            if (valueTuples[i].Length != arity)
                return;
        }

        if (arity == destinationColumns.Length)
            return;
        if (hasExplicitColumnList)
        {
            throw arity > destinationColumns.Length
    ...
    /// <summary>
    /// Mimics <strong>Msg 10709</strong>: the tuples of one <c>VALUES</c>
    /// constructor must all be the same width.
    /// </summary>
    private static void RejectRaggedValueTuples(List<Expression[]> valueTuples)
    {
        var arity = valueTuples[0].Length;
        for (var i = 1; i < valueTuples.Count; i++)
        {
    ...
    /// <summary>
    /// Rejects an explicit value landing in a column the engine writes itself
    /// — <strong>Msg 273</strong> for <c>rowversion</c>, <strong>Msg 13536</strong>
    /// for a <c>GENERATED ALWAYS</c> period column. Both columns are legal to
    /// name (and occupy a position in the implicit list); what they refuse is a
    /// value other than the <c>DEFAULT</c> keyword, which is the escape hatch
    /// each message's own text points at. A <c>SELECT</c> / <c>EXEC</c> source
    /// carries no DEFAULT keyword, so merely reaching such a column refuses.
    /// </summary>
    private static void RejectExplicitAutoGeneratedValues(
        HeapTable destinationTable,
        HeapColumn[] destinationColumns,
        List<Expression[]>? valueTuples,
        ParserContext context)
    {
        for (var i = 0; i < destinationColumns.Length; i++)
        {
            var column = destinationColumns[i];
            if (column.Type != SqlType.RowVersion && column.GeneratedAs == GeneratedAlwaysAsRow.None)
                continue;
            // Any one tuple supplying a real value is enough — real reports the
            // statement, not the row (probe-confirmed on a multi-row VALUES
            // whose first tuple defaulted the column and second didn't).
            var supplied = valueTuples is null;
            if (valueTuples is not null)
            {
        ...
        QueryPosition position = QueryPosition.InsertSource)
    {
        ...
        if (selection.Schema.Length != expectedColumnCount)
        {
            if (hasExplicitColumnList)
            {
                throw selection.Schema.Length < expectedColumnCount
                    ? SimulatedSqlException.InsertSelectListFewerThanInsertList()
                    : SimulatedSqlException.InsertSelectListMoreThanInsertList();
            }
    ...
    /// can't source — MERGE has no DEFAULT-keyword position to fill, so a
    /// derived, rowversion or identity projection is skipped rather than
    /// occupying a slot.
    /// </summary>
    private static HeapColumn[] BuildMergeInsertColumnsForView(View destinationView, HeapTable baseTable)
    {
        ...
        for (var i = 0; i < destinationView.OutputColumns.Length; i++)
        {
            ...
            if (col.Computed is not null || col.Type == SqlType.RowVersion || col.Identity is not null)
                continue;
    ...
    private static HeapColumn[] BuildImplicitInsertColumnsForView(View destinationView, HeapTable baseTable)
    {
        ...
        {
            var baseOrd = destinationView.BaseColumnOrdinals[i];
            // A derived projection has no base column to position a value
            // against, so real refuses the whole column-list-less form rather
            ...
            // keeps its slot and takes DEFAULT, as it does on a table target.
            if (col.Computed is not null || col.Identity is not null)
                continue;
            implicitList.Add(col);
        }
        return [.. implicitList];
