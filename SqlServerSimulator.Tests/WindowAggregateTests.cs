...
/// (...) OVER (PARTITION BY ...)</c>). The simulator supports the
/// implicit-frame whole-partition default; ORDER BY inside OVER for
/// aggregates raises <see cref="NotSupportedException"/> rather than
/// silently picking the wrong frame semantics. Probe-confirmed against
/// SQL Server 2025 (2026-05-08).
/// </summary>
[TestClass]
public sealed class WindowAggregateTests
{
    ...
    public void Sum_OverPartitionBy_BroadcastsTotalAcrossDetailRows()
    {
        ...
        // east: 100 + 200 + 150 = 450 (all 4 rows)
        // west: 300 + 500 = 800 (all 3 rows)
    ...
    public void Sum_OverEmptyPartition_BroadcastsGrandTotal()
    {
        using var connection = SeededSales();
        using var reader = connection.CreateCommand(
            "select sum(amount) over() from sales").ExecuteReader();
        var values = new List<decimal>();
        while (reader.Read())
            values.Add(reader.GetDecimal(0));
        HasCount(7, values);
        // All 7 rows see the grand total of 100+200+150+300+500 = 1250.
    ...
    public void Avg_DecimalScale_WidensToScale6OrMoreLikePlainAvg()
    {
        ...
        IsTrue(reader.Read());
        // (1.23 + 4.56) / 2 = 2.895; AVG widens decimal(5,2) → decimal(38, max(2,6)) = decimal(38,6).
    ...
    public void Avg_Int_TruncatesPerPartition()
    {
        ...
        using var reader = connection.CreateCommand(
            "select avg(v) over(partition by g) from t").ExecuteReader();
        var values = new List<int>();
        while (reader.Read()) values.Add(reader.GetInt32(0));
        HasCount(3, values);
        // (1+2+2)/3 = 5/3 = 1 (integer truncation); broadcast to all 3 rows.
        foreach (var v in values)
            AreEqual(1, v);
    }
    ...
    public void Count_Column_OverPartitionBy_SkipsNulls()
    {
        ...
        while (reader.Read())
            byRegion[reader.GetString(0)] = reader.GetInt32(1);
        // east: 4 rows, but 1 amount is NULL → 3 non-null
        // west: 3 rows, 1 amount is NULL → 2 non-null
    ...
    public void Stdev_OverPartitionBy_Float()
    {
        ...
        IsTrue(reader.Read());
        // east non-null amounts: 100, 200, 150 → sample stdev = 50
    ...
    public void PartitionBy_MultiKey_SubdividesPartitions()
    {
        ...
        // (east, alice, 100+200=300) appears twice
        // (east, bob, 150 + null=150) appears twice
        // (west, carol, 300+500=800) appears twice
        // (west, dan, null) appears once → sum is null (all-null partition)
    ...
    {
        // Projection-alias ORDER BY isn't modeled by the simulator (a
        // pre-existing gap); positional ORDER BY (`order by 2 desc`)
        // works and reaches the same result.
        using var connection = SeededSales();
        using var reader = connection.CreateCommand(
        ...
        // First three rows are the west region (sum 800); last four are east (sum 450).
    ...
    public void Sum_BigInt_StaysBigInt()
    {
        ...
        using var reader = connection.CreateCommand(
            "select sum(v) over() from b").ExecuteReader();
        IsTrue(reader.Read());
    ...
    [TestMethod]
    public void Sum_Distinct_Over_RaisesMsg10759()
    {
        using var connection = SeededSales();
        var ex = Throws<DbException>(() =>
            _ = connection.CreateCommand("select sum(distinct amount) over(partition by region) from sales").ExecuteScalar());
        AreEqual("10759", ex.Data["HelpLink.EvtID"]);
    }

    [TestMethod]
    public void Count_Distinct_Over_RaisesMsg10759()
    {
        using var connection = SeededSales();
        var ex = Throws<DbException>(() =>
            _ = connection.CreateCommand("select count(distinct salesperson) over(partition by region) from sales").ExecuteScalar());
        AreEqual("10759", ex.Data["HelpLink.EvtID"]);
    }

    [TestMethod]
    public void StringAgg_Over_RaisesMsg4113()
    {
        using var connection = SeededSales();
        var ex = Throws<DbException>(() =>
            _ = connection.CreateCommand("select string_agg(salesperson, ',') over(partition by region) from sales").ExecuteScalar());
        AreEqual("4113", ex.Data["HelpLink.EvtID"]);
    }

    [TestMethod]
    public void WindowedAggregate_InWhereClause_RaisesMsg4108()
    {
        using var connection = SeededSales();
        var ex = Throws<DbException>(() =>
            _ = connection.CreateCommand("select region from sales where sum(amount) over(partition by region) > 100").ExecuteScalar());
        AreEqual("4108", ex.Data["HelpLink.EvtID"]);
    }

    [TestMethod]
    public void WindowedAggregate_InHavingClause_RaisesMsg4108()
    {
        using var connection = SeededSales();
        var ex = Throws<DbException>(() =>
            _ = connection.CreateCommand("select region from sales group by region having sum(amount) over(partition by region) > 100").ExecuteScalar());
        AreEqual("4108", ex.Data["HelpLink.EvtID"]);
    }

    [TestMethod]
    public void WindowedAggregate_InGroupByClause_RaisesMsg4108()
    {
        using var connection = SeededSales();
        var ex = Throws<DbException>(() =>
            _ = connection.CreateCommand("select region from sales group by sum(amount) over(partition by region)").ExecuteScalar());
        AreEqual("4108", ex.Data["HelpLink.EvtID"]);
    }

    [TestMethod]
    public void OrderByInsideOver_ForAggregate_NotSupported()
    {
        using var connection = SeededSales();
        _ = Throws<NotSupportedException>(() =>
            _ = connection.CreateCommand("select sum(amount) over(partition by region order by amount) from sales").ExecuteScalar());
    }

    [TestMethod]
    public void RowsBetweenFrame_NotSupported()
    {
        using var connection = SeededSales();
        _ = Throws<NotSupportedException>(() =>
            _ = connection.CreateCommand("select sum(amount) over(partition by region rows between unbounded preceding and current row) from sales").ExecuteScalar());
    }

    [TestMethod]
    public void RangeBetweenFrame_NotSupported()
    {
        using var connection = SeededSales();
        _ = Throws<NotSupportedException>(() =>
            _ = connection.CreateCommand("select sum(amount) over(partition by region range between unbounded preceding and current row) from sales").ExecuteScalar());
    }

    [TestMethod]
    public void AggregateWindow_CombinedWithGroupBy_NotSupported()
    {
        using var connection = SeededSales();
        _ = Throws<NotSupportedException>(() =>
            _ = connection.CreateCommand("select region, sum(amount), sum(amount) over() from sales group by region").ExecuteScalar());
    }
}
